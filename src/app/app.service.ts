...
export class AppService extends AppServiceBase {
    ...
    public toolbarIsHidden: BehaviorSubject<boolean> =
        new BehaviorSubject<boolean>(false);
    public toolbarIsHidden$: Observable<boolean> =
        this.toolbarIsHidden.asObservable();
    public narrowingPageContentWhenFixedFilter = true;
    public hideSubscriptionCallback: Function;
    private showContactInfoPanel: BehaviorSubject<boolean> =
        new BehaviorSubject<boolean>(false);
    public showContactInfoPanel$: Observable<boolean> =
        this.showContactInfoPanel.asObservable();
    public clientSearchPhrase: BehaviorSubject<string> =
        new BehaviorSubject<string>("");
    public clientDetailsChange: BehaviorSubject<string> =
        new BehaviorSubject<string>("");
    public clientSearchToggle: BehaviorSubject<boolean> =
        new BehaviorSubject<boolean>(false);
    ...
    constructor(
                ...
                map((pipelines: PipelineDto[]) => {
                    return pipelines.map((pipeline: PipelineDto) => {
                        let contactGroup = Object.keys(ContactGroup).find(
                            (key) =>
                                ContactGroup[key] == pipeline.contactGroupId
                        );
                        return {
                            title: pipeline.name,
                            route: "/app/crm/leads",
                            permission:
                                AppPermissions["CRM" + contactGroup + "s"],
                            params: {
        ...
        this.toolbarIsHidden$.subscribe((hidden: boolean) => {
            this.document.body.classList[hidden ? "add" : "remove"](
                "toolbar-hidden"
            );
        });
    }
    ...
    loadModuleSubscriptions() {
        this.moduleSubscriptions$ = this.tenantSubscriptionProxy
        ...
        this.moduleSubscriptions$.subscribe(
            (res: ModuleSubscriptionInfoDto[]) => {
                this.moduleSubscriptions = res.sort(
                    (
                        left: ModuleSubscriptionInfoDto,
                        right: ModuleSubscriptionInfoDto
                    ) => {
                        const isLeftSignupGroup =
                                left.productGroup &&
                                this.checkSignUpOrMainGroups(left.productGroup),
                            isRightSignupGroup =
                                right.productGroup &&
                                this.checkSignUpOrMainGroups(
                                    right.productGroup
                                );
                        if (
                        ...
                        else if (isLeftSignupGroup) return -1;
                        else return 1;
                    }
                );
            }
    ...
    checkSignUpOrMainGroups(group: string): Boolean {
        return this.checkGroupIncluded(
            [AppConsts.PRODUCT_GROUP_SIGNUP, AppConsts.PRODUCT_GROUP_MAIN],
            group
        );
    }
    ...
    checkSubscriptionsGroups(productGroups: string[]) {
        return this.moduleSubscriptions.some(
            (item) =>
                item.productGroup &&
                this.checkGroupIncluded(productGroups, item.productGroup)
        );
    }
    ...
    getModuleSubscription(
        ...
        let module = name.toUpperCase(),
            moduleSubscriptions: ModuleSubscriptionInfoDto[] =
                this.moduleSubscriptions && productGroups.length
                    ? this.moduleSubscriptions.filter(
                          (item) =>
                              item.productGroup &&
                              this.checkGroupIncluded(
                                  productGroups,
                                  item.productGroup
                              )
                      )
        ...
        if (hasCrmFeature && moduleSubscriptions)
            moduleSubscriptions = moduleSubscriptions.filter(
                (item) => item.hasCrmFeature
            );

        if (moduleSubscriptions && moduleSubscriptions.length) {
            subscription = _.find(
                moduleSubscriptions,
                (subscription: ModuleSubscriptionInfoDto) => {
                    return (
                        subscription.module.includes(module) &&
                        subscription.statusId == "A"
                    );
                }
            );
            if (!subscription)
    ...
    getSubscriptionStatusByModuleName(
        moduleName: string = this.defaultSubscriptionModule
    ) {
        return this.getSubscriptionStatusBySubscription(
            this.getModuleSubscription(moduleName)
        );
    }

    getSubscriptionStatusBySubscription(
        subscription: ModuleSubscriptionInfoDto
    ) {
    ...
    isOneTimeExpirationSoon(
        name: string = this.defaultSubscriptionModule
    ): boolean {
        let sub = this.getModuleSubscription(name);
        return (
            sub.paymentPeriodType == PaymentPeriodType.OneTime &&
            this.subscriptionIsExpiringSoon(name)
        );
    }
    ...
    subscriptionIsExpiringSoon(
        ...
        if (!this.isHostTenant && sub && sub.endDate) {
            ...
            return (
                diff > 0 && diff <= AppConsts.subscriptionExpireNootifyDayCount
            );
        }
        return false;
    }
    ...
    subscriptionInGracePeriodBySubscription(
        sub: ModuleSubscriptionInfoDto
    ): boolean {
    ...
    getGracePeriod(sub: ModuleSubscriptionInfoDto) {
        return sub.endDate && sub.finalEndDate
            ? sub.finalEndDate.diff(sub.endDate, "days", true)
            : 0;
    }
    ...
    getSubscriptionExpiringDayCount(
        ...
        return (
            sub &&
            sub.endDate &&
            Math.round(moment(sub.endDate).diff(moment().utc(), "days", true))
        );
    }
    ...
    getGracePeriodDayCountBySubscription(sub) {
        return (
            ...
            Math.round(
                moment(sub.endDate)
                    .add(this.getGracePeriod(sub), "days")
    ...
    hasModuleSubscription(
        ...
        let module = this.getModuleSubscription(
            name,
            productGroups,
            hasCrmFeature
        );

        if (module && ["D", "C"].includes(module.statusId)) return false;

        return (
            this.isHostTenant ||
            !module ||
            !module.endDate ||
            this.hasRecurringBilling(module) ||
    ...
    hasRecurringBilling(module: ModuleSubscriptionInfoDto): boolean {
        return (
            module &&
            module.hasRecurringBilling &&
            moment(module.endDate).add(
    ...
    hasUnconventionalSubscription() {
        ...
        if (!sub.productId) {
            ...
            else return hasActiveSubscription;
        }
        return false;
    }
    ...
    canSendVerificationRequest() {
        return this.permission.isGranted(
            AppPermissions.CFOMembersAdministrationNewMemberRegistration
        );
    }
            ...
            abp.message.confirm(
                ...
                (isConfirmed) => {
                    if (isConfirmed) {
                        ...
                        this.instanceServiceProxy
                            .registerMember(request)
                            .subscribe(
                                (result) => {
                                    abp.notify.info(
                                        "User was activated and email sent successfully"
                                    );
    ...
    redirectToCFO(userId: number) {
        this.instanceServiceProxy
            .getUserInstanceInfo(userId)
            .subscribe((result: GetUserInstanceInfoOutput) => {
                if (
                    result &&
                    result.id &&
                    result.status === InstanceStatus.Active
                )
                    window.open(
                    ...
                    this.notify.error(
                        this.appLocalizationService.ls(
    ...
    checkCFOClientAccessPermission() {
        return this.permission.isGranted(
            AppPermissions.CFOMembersAdministrationAllMemberInstancesAdmin
        );
    }
    ...
    isFeatureEnable(featureName: AppFeatures | string): boolean {
        return (
            this.isHostTenant ||
            !featureName ||
            featureName.split("&").every((item) => {
                let itemValue: any = this.feature.getValue(item);
                if (isNaN(itemValue)) return this.feature.isEnabled(item);
                else {
    ...
    getFeatureCount(feature: AppFeatures, forced = false) {
        return !forced && this.isHostTenant
            ? AppConsts.infinityFeatureCount
            : +abp.features.getValue(feature);
    }
}
