  ...
  implements AfterViewInit, OnInit, OnDestroy
{
  ...
  public validImageExtensions: String[] = ['jpeg', 'jpg', 'jfif', 'jpe', 'png', 'bmp', 'gif'];
    ...
    this.viewerToolbarConfig = [
      {
        ...
        items: [
          ...
          },
          {
            ...
            )} ${this.getFileType(this.currentDocumentInfo.fileName)}" title="${
              this.currentDocumentInfo.fileName
            } ${this.fileSizePipe.transform(this.currentDocumentInfo.size)}">
          ...
          },
          {
            ...
            action: () => {
              ...
              if (
                ...
                const printSrc =
                  this.showViewerType == DocumentViewerType.IMAGE
                    ? this.imageViewer.images[0]
                    : viewedDocument.textContent;
                const format = <any>(
                  this.getFileExtensionByFileName(this.currentDocumentInfo.fileName)
                );
  ...
  toggleActionsMenu(data, target) {
    ...
    this.actionMenuItems.find(menuItem => menuItem.text === this.l('Edit')).visible =
      data.isEditSupportedByWopi;
    ...
    if (viewerType !== DocumentViewerType.UNKNOWN) {
      ...
      switch (viewerType) {
        case DocumentViewerType.WOPI:
          this.message.confirm('', this.l('Do you want to download this file?'), isConfirmed => {
...
*/
        ...
        case DocumentViewerType.VIDEO:
          this.documentsService
            .getDocumentUrlInfoObservable(this.currentDocumentInfo.id)
            .subscribe(urlInfo => {
              super.finishLoading(true);
              ...
              RequestHelper.downloadFileBlob(
                ...
                blob => {
                  if (viewerType === DocumentViewerType.ARCHIVE) {
                    this.archiveFiles$ = (
                      ext === 'rar'
                        ? this.getFilesInfoFromRarBlob(blob)
                        : this.getFilesInfoFromZipBlob(blob)
                    ).pipe(tap(() => (this.openDocumentMode = true)));
                  } else {
                    ...
                    reader.addEventListener('loadend', () => {
                      ...
                      if (viewerType === DocumentViewerType.XML) {
                        const json = xmlJs.xml2js(this.sanitizeContent(this.previewContent), {
                          compact: true,
                          trim: true,
                          ignoreDoctype: true,
  ...
  matchesSearch(document: any, fields: string[], search: string): boolean {
    return fields.some(field => (document[field] || '').toLowerCase().includes(search));
  }
        ...
        items: Object.keys(DocumentTabsFilter).map(key => {
          return {
            ...
            options: {
              text: key,
              checkPressed: () => {
                return this.selectedDocumentTabsFilter == DocumentTabsFilter[key];
              },
            },
            action: () => {
        ...
        items: [
          {
            ...
            options: {
              ...
              items: ['', ...this.allValidExtensions].map(extension => {
                return {
        ...
        items: [
          {
            ...
            options: {
              ...
              width: '263',
              mode: 'search',
              placeholder: this.l('Search') + ' ' + this.l('files and documents').toLowerCase(),
              onValueChanged: e => {
        ...
        items: [
          {
            ...
            attr: {
              class: `dx-document-grid-view ${
                this.selectedViewType === DocumentViewTypeFilter.Grid
                  ? 'dx-document-selected-view'
                  : ''
              }`,
            },
          },
