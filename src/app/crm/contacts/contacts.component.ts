...
export class ContactsComponent extends AppComponentBase implements OnDestroy {
  ...
  isCommunicationHistoryAllowed$: Observable<boolean> = this.contactsService.contactInfo$.pipe(
    map(
      (contactInfo: ContactInfoDto) =>
        contactInfo &&
        this.permission.checkCGPermission(contactInfo.groups, 'ViewCommunicationHistory')
    )
  );
  ...
  navLinks: NavLink[] = [
      ...
      label$: this.contactsService.leadInfo$.pipe(
        map(lead => {
          if (lead && lead.typeSysId) {
            ...
            if (lead.typeSysId.startsWith(EntityTypeSys.PropertyRentAndSale))
    ...
    },
    {
      ...
      label$: this.userId$.pipe(
        map((userId: number) => (userId ? this.l('UserInformation') : this.l('InviteUser')))
      ),
      visible$: combineLatest(this.userId$, this.contactsService.contactInfo$).pipe(
        map(([userId, contactInfo]: [number, ContactInfoDto]) => {
          return userId
            ? this.permission.isGranted(AppPermissions.AdministrationUsersEdit) ||
                (contactInfo &&
    ...
    },
    {
      ...
      visible$: this.leadInfo$.pipe(map((leadInfo: LeadInfoDto) => !!leadInfo.id)),
      disabled: true,
    },
    {
      name: 'questionnaire',
  ...
  initNavButtons() {
    ...
    const itemKeyField = this.dataSourceURI == ItemTypeEnum.User ? 'id' : 'Id',
      itemDistinctField =
        [ItemTypeEnum.Order, ItemTypeEnum.Subscription].indexOf(this.dataSourceURI) >= 0
          ? 'LeadId'
          : itemKeyField;
    let subscription = this.targetEntity$
      ...
      .subscribe((itemFullInfo: ItemFullInfo) => {
        ...
        if (
          isImproperItemInfo &&
          itemFullInfo.items.some(item => this.currentItemId == item[itemKeyField])
        ) {
  ...
  private fillContactDetails(result: ContactInfoDto, contactId = null) {
    ...
    this.isCommunicationHistoryAllowed =
      ...
      this.permission.checkCGPermission(result.groups, 'ViewCommunicationHistory');
    this.isSendSmsAndEmailAllowed =
      featureMaxMessageCount &&
      this.permission.checkCGPermission(result.groups, 'ViewCommunicationHistory.SendSMSAndEmail');
  ...
  getContactInfoWithCompany(
    ...
    return forkJoin(
      ...
      map(
        ([contactInfo, organizationContactInfo]: [ContactInfoDto, OrganizationContactInfoDto]) => {
          ...
          if (!companyId && contactInfo.primaryOrganizationContactId)
            this.orgContactService
              .getOrganizationContactInfo(contactInfo.primaryOrganizationContactId)
          ...
          else this.contactsService.organizationInfoUpdate(OrganizationContactInfoDto.fromJS({}));
          return contactInfo;
        }
      )
    );
      ...
      contactInfo$
        .pipe(
          ...
          switchMap((result: ContactInfoDto) => {
            ...
            if (leadId) {
              this.loadLeadsStages();
              if (!this.contactInfo['leadId']) this.contactInfo['leadId'] = leadId;
            }
  ...
  loadLeadData(personContactInfo?: PersonContactInfoDto, lastLeadCallback?) {
    ...
    if (
      (contactInfo && (!leadInfo || !this.leadInfo || this.leadInfo.id != leadInfo.id)) ||
      lastLeadCallback
    ) {
      this.contactGroupId$.pipe(filter(Boolean), first()).subscribe((contactGroupId: string) => {
        ...
        let leadId = leadInfo && leadInfo.id,
          leadInfo$ =
            leadId && !lastLeadCallback
              ...
              : this.leadService.getLastLeadInfo(contactGroupId, contactInfo.id);
        ...
        leadInfo$
          ...
          .subscribe(successCallback, error => {
            if (error.code == 404)
              this.leadService
                .getLastLeadInfo(contactGroupId, contactInfo.id)
    ...
    this.leadInfo$
      .pipe(
        ...
        switchMap(leadInfo => {
          return zip(
            of(leadInfo),
            this.pipelineService.getAllPipelinesOberverable(AppConsts.PipelinePurposeIds.lead)
          );
        })
      )
            ...
            return {
              ...
              items: pipeline.stages.map((stage: StageDto) => {
                return {
                  ...
                  disabled:
                    !leadCGManageAllowed || (leadInfo.pipelineId != pipeline.id && stage.isFinal),
  ...
  showContactPersons = (event, posLeft = -182) => {
    ...
    this.dialog
      .open(ContactPersonsDialogComponent, {
        ...
        panelClass: ['related-contacts', ...(this.showModernLayout ? ['modern'] : [])],
      })
      .afterClosed()
      .subscribe(result => {
        if (result == 'addContact') this.addNewContact(event);
          ...
          this.contactService
            ...
            .subscribe(contactInfo => {
              ...
              let orgContactInfo = (contactInfo['organizationContactInfo'] =
  ...
  close(force: boolean = false) {
    ...
    this._router.navigate([(this.queryParams && this.queryParams.referrer) || 'app/crm/clients'], {
      queryParams: _.extend(queryParams, refresh ? { refresh: Date.now() } : {}),
    });
    delete data.refresh;
  }
      ...
      .subscribe((pipelineId: number) => {
        if (pipelineId == $event.itemData.pipelineId)
          ...
          this.message.confirm(
            ...
            this.l('AreYouSure'),
            confirmed => {
              if (confirmed) this.updateStageInternal(pipelineId, $event.itemData);
              else this.toolbarComponent.stagesComponent.disabled = false;
            }
          );
  ...
  private updateStageInternal(pipelineId, data) {
    ...
    if (
      !this.pipelineService.updateEntityStage(this.leadInfo, sourceStage, targetStage, () => {
  ...
  refreshStageDropdown() {
    if (this.leadInfo)
      this.toolbarComponent.stagesComponent.listComponent.option('selectedItemKeys', [
        this.leadInfo.stageId,
      ]);
    ...
    this.message.confirm(
      ...
      isConfirmed => {
        if (isConfirmed) {
          ...
          this.toolbarComponent.partnerTypesComponent.listComponent.option('selectedItemKeys', [
  ...
  getAssignmentsPermissionKey = () => {
    return this.contactGroupId.value
      ? this.permission.getCGPermissionKey([this.contactGroupId.value], 'ManageAssignments')
      : '';
  };
