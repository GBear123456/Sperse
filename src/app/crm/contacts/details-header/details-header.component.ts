...
export class DetailsHeaderComponent implements OnInit, OnDestroy {
    ...
    addContextComponent: DxContextMenuComponent;
    ...
    private contactInfo: BehaviorSubject<ContactInfoDto> =
        new BehaviorSubject<ContactInfoDto>(new ContactInfoDto());
    contactInfo$: Observable<ContactInfoDto> = this.contactInfo.asObservable();

    private _personContactInfo: BehaviorSubject<PersonContactInfoDto> =
        new BehaviorSubject<PersonContactInfoDto>(new PersonContactInfoDto());
    personContactInfo$: Observable<PersonContactInfoDto> =
        this._personContactInfo.asObservable();
    selectedOrganizationId: ReplaySubject<number> = new ReplaySubject<number>(
        1
    );
    selectedOrganizationId$: Observable<number> =
        this.selectedOrganizationId.asObservable();
    selectedOrganizationInfo$: Observable<PersonOrgRelationShortInfo> =
        combineLatest(
            this.personContactInfo$,
            this.selectedOrganizationId$
        ).pipe(
            map(
                ([personContactInfo, selectedRelationId]: [
                    PersonContactInfoDto,
                    number
                ]) => {
                    return (
                        (personContactInfo.orgRelations &&
                            personContactInfo.orgRelations.find(
                                (orgRelation: PersonOrgRelationShortInfo) => {
                                    return (
                                        orgRelation.id === selectedRelationId
                                    );
                                }
                            )) ||
    ...
    personJobTitleInSelectedOrganization$: Observable<string> =
        this.selectedOrganizationInfo$.pipe(
            map(
                (personOrganizationInfo: PersonOrgRelationShortInfo) =>
                    personOrganizationInfo && personOrganizationInfo.jobTitle
            )
        );
    contactId: number;
    leadId: number;
    ...
    private readonly allContactGroupsExceptUser = this.allContactGroups.filter(
        (v) => v != ContactGroup.Employee
    );

    featureMaxMessageCount = this.contactsService.getFeatureCount(
    ...
    manageAllowed$: Observable<boolean> = this.contactInfo$.pipe(
        ...
        map((contactInfo: ContactInfoDto) =>
            this.permissionService.checkCGPermission(contactInfo.groups)
        )
    );
    ...
    nameValidationRules = [
        ...
        {
            type: "pattern",
            pattern: AppConsts.regexPatterns.fullName,
            message: this.ls.l("FullNameIsNotValid"),
        },
    ];
    ...
    ngOnInit(): void {
        ...
        this.contextMenuInit$.subscribe(
            ([contactInfo, propertyId, manageAllowed]: [
                ContactInfoDto,
                number,
                boolean
            ]) => {
                this.addContextMenuItems = this.getDefaultContextMenuItems(
                    manageAllowed,
                    !!propertyId
                ).filter((menuItem: ContextMenuItem) => {
                    return (
                        contactInfo.groups &&
                        contactInfo.groups.some(
                            (group) =>
                                menuItem.contactGroups.indexOf(group.groupId) >=
                                0
                        )
                    );
                });
        ...
        zip(
            ...
            this.personContactInfo$.pipe(
                filter(
                    (contact: PersonContactInfoDto) =>
                        !!(contact && contact.orgRelations)
                )
            ),
            ...
            .pipe(first())
            .subscribe(
                ([contactInfo, personContactInfo, params]: [
                    ContactInfoDto,
                    PersonContactInfoDto,
                    Params
                ]) => {
                    let companyId = isNaN(params["companyId"])
                    ...
                    const selectedRelation =
                        personContactInfo.orgRelations.find(
                            (orgRelation: PersonOrgRelationShortInfo) => {
                                return (
                                    orgRelation.organization.id === companyId
                                );
                            }
                        );
        ...
        this.contactInfo$
            ...
            .subscribe((contactInfo: ContactInfoDto) => {
                ...
                this.companyName =
                    contactInfo["organizationContactInfo"]?.fullName || "";
    ...
    getDefaultContextMenuItems(
        ...
        return [
            ...
            },
            {
                ...
                visible:
                    !this.data.parentId &&
                    this.permissionChecker.isGranted(
                        AppPermissions.CRMOrdersInvoicesManage
                    ),
                contactGroups: this.allContactGroups,
            },
            {
                ...
                visible:
                    ...
                    manageAllowed &&
                    this.permissionChecker.isGranted(
                        AppPermissions.CRMOrdersManage
                    ) &&
                    !!this.contactsService.getFeatureCount(
                        AppFeatures.CRMMaxProductCount
                    ) &&
    ...
    initializePersonOrgRelationInfo(
        ...
        if (personContactInfo && personContactInfo.orgRelations) {
            ...
            if (
                !orgRelation ||
                orgRelation.id != personContactInfo.orgRelationId
            )
                personContactInfo["personOrgRelationInfo"] =
                    personContactInfo.orgRelations.find(
                        (orgRelation) =>
    ...
    updateJobTitle(value) {
        ...
        this.personOrgRelationService
            .update(
                UpdatePersonOrgRelationInput.fromJS({
    ...
    removePersonOrgRelation(event) {
        ...
        this.messageService.confirm(
            ...
            (result: boolean) => {
                if (result) {
                    let orgRelationId =
                        this.personContactInfo["personOrgRelationInfo"].id;
                    this.showRemovingOrgRelationProgress = true;
                    this.personOrgRelationService
                        .delete(orgRelationId)
                        .pipe(
                            finalize(
                                () =>
                                    (this.showRemovingOrgRelationProgress =
                                        false)
                            )
                        )
                        .subscribe(() => {
                            ...
                            let orgRelationToDelete = _.find(
                                orgRelations,
                                (orgRelation) =>
                                    orgRelation.id === orgRelationId
                            );
                            if (
                                this.data.primaryOrganizationContactId ==
                                orgRelationToDelete.organization.id
                            ) {
                                this.data["organizationContactInfo"] =
                                    undefined;
                                this.data.primaryOrganizationContactId =
                                    orgRelations.length
                                        ? orgRelations.reverse()[0].organization
                                              .id
                                        : undefined;
                            }
                            orgRelations.splice(
                                orgRelations.indexOf(orgRelationToDelete),
                                1
                            );
                            this.displayOrgRelation(
                                orgRelationToDelete.organization.id
                            );
    ...
    showCompanyDialog(e) {
        ...
        if (!companyInfo || !companyInfo.id) return this.addCompanyDialog(e);
        ...
        this.dialog
            ...
            .subscribe((result) => {
                if (result) {
                    companyInfo.organization = new OrganizationInfoDto(
                        result.company
                    );
    ...
    displayOrgRelation(orgId) {
        ...
        if (this.data.primaryOrganizationContactId == orgId) {
            let orgRelation =
                orgRelations &&
                _.sortBy(orgRelations, (orgRelation) => {
                    return orgRelation.id;
                }).reverse()[0];
            if (orgRelation) {
                orgRelation.isPrimary = true;
                this.data.primaryOrganizationContactId =
                    orgRelation.organization.id;
                this.displaySelectedCompany(
                    orgRelation.organization.id,
                    orgRelation.id
                );
            } else {
            ...
            let orgRelation =
                _.find(orgRelations, (item) => item.isPrimary) ||
                orgRelations[0];
    ...
    getPhoto(primaryPhoto: string): string {
        return primaryPhoto
            ? "url(data:image/png;base64," +
                  this.data.personContactInfo.primaryPhoto +
                  ")"
            : "url(" +
                  this.profileService.getContactPhotoUrl(null, true, "large") +
                  ")";
    ...
    showUploadPhotoDialog(event, isCompany?: boolean) {
        if (!this.manageAllowed) return false;
        ...
        this.dialog
            ...
            .subscribe((result: UploadPhotoResult) => {
                let dataField =
                    (isCompany ? "primaryOrganization" : "person") +
                    "ContactInfo";
                if (result.clearPhoto) {
                    this.contactPhotoServiceProxy
    ...
    showPersonHistory(event) {
        event.stopPropagation();
        this.dialog
            .open(PersonHistoryDialogComponent, {
                panelClass: "slider",
                disableClose: true,
    ...
    updatePrimaryContactName(value) {
        ...
        this.personContactServiceProxy
            .updatePersonName(
    ...
    get addOptionCacheKey() {
        return (
            this.ADD_OPTION_CACHE_KEY +
            "_" +
            this.data.groups.map((group) => group.groupId).join("_")
        );
    }

    addOptionsInit() {
        if (this.addContextMenuItems.length) {
            let cacheKey = this.cacheHelper.getCacheKey(
                    this.addOptionCacheKey,
    ...
    addOptionSelectionChanged(event) {
        const option: ContextMenuItem =
            event.addedItems.pop() ||
            event.removedItems.pop() ||
    ...
    addEntity(event?) {
        ...
        const selectedMenuItem = this.addContextMenuItems.find(
        ...
        if (selectedMenuItem.type === ContextType.AddContact)
            ...
            let leadId = this.activatedRoute.parent.snapshot.paramMap.get(
                "leadId"
            )
                ? this.leadId
                : null;
    ...
    private getContextMenuItemByType(
        contextType: ContextType
    ): ContextMenuItem {
        return this.addContextMenuItems.find(
            (contextMenuItem: ContextMenuItem) => {
    ...
    addCompanyDialog(event) {
        if (this.manageAllowed)
            this.contactsService
                .addCompanyDialog(event, this.data)
                .pipe(
    ...
    addCompanyLogo(event) {
        if (this.isOrgUpdatable) {
            const organizationContactInfo: OrganizationContactInfoDto =
                this.data["organizationContactInfo"];
            this.contactsService
    ...
    showCompanyList(event) {
        this.dialog.closeAll();
        this.dialog
            .open(RelationCompaniesDialogComponent, {
                data: this.data,
                hasBackdrop: false,
                position: DialogService.calculateDialogPosition(
                    event,
            ...
            .afterClosed()
            .subscribe((result) => {
                if (result == "addContact") this.addCompanyDialog(event);
                else if (result)
    ...
    displaySelectedCompany(orgId: number, orgRelationId: number) {
        ...
        this.loadingService.startLoading();
        this.orgContactService
            .getOrganizationContactInfo(orgId)
            .pipe(finalize(() => this.loadingService.finishLoading()))
    ...
    showEmailDialog() {
        this.contactsService
            .showEmailDialog({ contact: this.data })
            .subscribe();
