...
export class ContactsService {
    ...
    private leadInfoSubject: BehaviorSubject<LeadInfoDto> =
        new BehaviorSubject<LeadInfoDto>(undefined);
    leadInfo$: Observable<LeadInfoDto> = this.leadInfoSubject.asObservable();
    private contactInfo: ReplaySubject<ContactInfoDto> =
        new ReplaySubject<ContactInfoDto>(1);
    ...
    contactId$: Observable<number> = this.contactId
        .asObservable()
        .pipe(filter((contactId: number) => !!contactId));
    contactGroupId: BehaviorSubject<string> = new BehaviorSubject<string>(null);
    private organizationContactInfo: ReplaySubject<OrganizationContactInfoDto> =
        new ReplaySubject<OrganizationContactInfoDto>(1);
    organizationContactInfo$: Observable<OrganizationContactInfoDto> =
        this.organizationContactInfo.asObservable();
    private subscribers: any = {
        common: [],
    };
    private personContactInfo: ReplaySubject<PersonContactInfoDto> =
        new ReplaySubject(1);
    personContactInfo$: Observable<PersonContactInfoDto> =
        this.personContactInfo.asObservable();
    readonly settingsDialogOpenedCacheKey: string =
        this.cacheHelper.getCacheKey("save_option_opened_settings");
    settingsDialogOpened: BehaviorSubject<boolean> = new BehaviorSubject(
        this.cacheService.get(this.settingsDialogOpenedCacheKey) || true
    );
    settingsDialogOpened$: Observable<boolean> = this.settingsDialogOpened
        .asObservable()
    ...
    private subscribe(sub, ident: string = "common") {
        if (!this.subscribers[ident]) this.subscribers[ident] = [];
    ...
    toggleSettingsDialog() {
        if (this.settingsDialogOpened.value) this.closeSettingsDialog();
        else this.openSettingsDialog();
    }

    contactInfoSubscribe(callback, ident?: string) {
        return this.subscribe(
            this.contactInfo.asObservable().subscribe(callback),
            ident
        );
    }
    ...
    organizationInfoSubscribe(callback, ident?: string) {
        return this.subscribe(
            this.organizationContactInfo.asObservable().subscribe(callback),
            ident
        );
    }
    ...
    loadLeadInfoSubscribe(callback, ident?: string) {
        return this.subscribe(
            this.loadLeadInfoSubject.asObservable().subscribe(callback),
            ident
        );
    }
    ...
    addCompanyDialog(
        ...
        shiftX?,
        shiftY?
    ): Observable<CreatePersonOrgRelationOutput> {
    ...
    showUploadPhotoDialog(
        ...
        const uploadPhotoData: UploadPhotoData = {
            ...
            source: companyPhoto
                ? "data:image/jpeg;base64," + companyPhoto
                : null,
            ...
            .pipe(
                ...
                switchMap((result: UploadPhotoResult) => {
                    ...
                    if (result.clearPhoto) {
                        action$ = this.contactPhotoServiceProxy
                            .clearContactPhoto(companyId)
                            .pipe(mapTo(null));
                    } else {
                        let base64OrigImage = StringHelper.getBase64(
                            result.origImage
                        );
                        let base64ThumbImage = StringHelper.getBase64(
                            result.thumbImage
                        );
                        action$ = this.contactPhotoServiceProxy
                            .createContactPhoto(
    ...
    getContactInfo(contactId): Observable<ContactInfoDto> {
        let contactInfo =
            this.contactProxy["data"] && this.contactProxy["data"].contactInfo;
        return contactInfo && contactInfo.id == contactId
            ? of(contactInfo)
    ...
    initSuggestionEmails(emailData, title: string) {
        if (emailData.contact) {
            ...
            emailData.suggestionEmails =
                emailData.contact.personContactInfo.details.emails
                    .filter((item) => item.isActive)
                    .map((item) => item.emailAddress);

            if (
                emailData.suggestionEmails.length &&
                ["Reply", "ReplyToAll", "Forward", "Resend"].indexOf(title) < 0
            ) {
            ...
            emailData.contact.personContactInfo.details.phones
                ...
                .map((item) =>
                    emailData.suggestionEmails.push(
                        item.phoneNumber + "@fax.clicksend.com"
                    )
                );
        }
    }
    ...
    initEmailDialogTagsList(dialogComponent: EmailTemplateDialogComponent) {
        if (!dialogComponent.tagsList || !dialogComponent.tagsList.length) {
            dialogComponent.tagsList = this.getEmailTemplateTags(
                dialogComponent.data.templateType
            );
        }
    }
    ...
    getEmailTemplateTags(templateType: EmailTemplateType): any[] {
        switch (templateType) {
            ...
            case EmailTemplateType.WelcomeEmail:
                return [
                    WelcomeEmailTags.FirstName,
                    WelcomeEmailTags.LastName,
                    WelcomeEmailTags.UserEmail,
                    WelcomeEmailTags.Password,
                    WelcomeEmailTags.BaseUrl,
                    WelcomeEmailTags.SenderSystemName,
                    WelcomeEmailTags.SenderEmailSignature,
                    WelcomeEmailTags.AutologinLink,
                    WelcomeEmailTags.TrackingPixel,
                ];
            case EmailTemplateType.ProductPaid:
                return [
                    ProductEmailTags.ClientFullName,
                    ProductEmailTags.ProductName,
                    ProductEmailTags.ProductDescription,
                    ProductEmailTags.ProductQuantity,
                    ProductEmailTags.PriceUnitName,
                    ProductEmailTags.ProductInvoiceUrl,
                    ProductEmailTags.ProductReceiptUrl,
                ];
            default:
                return [];
        }
    ...
    showEmailTemplateDialog(templateId?: number): EmailTemplateDialogComponent {
        ...
        let dialogComponent = this.dialog.open(EmailTemplateDialogComponent, {
            ...
            data: {
                ...
                title: addMode
                    ? this.ls.l("Add Template")
                    : this.ls.l("Edit Template"),
    ...
    showEmailDialog(
        ...
        onTemplateChange?: (
            templateId: number,
            dialogComponent: EmailTemplateDialogComponent
        ) => Observable<void>
    ): Observable<number> {
        ...
        if (emailData.contact) this.initSuggestionEmails(emailData, title);
        else if (emailData["contactId"])
            this.getContactInfo(emailData["contactId"]).subscribe(
                (contactInfo: ContactInfoDto) => {
        ...
        if (emailData.templateType == EmailTemplateType.Contact)
            dialogComponent.onTemplateCreate.subscribe((templateId: number) => {
                const emailTemplateDialog =
                    this.showEmailTemplateDialog(templateId);
                emailTemplateDialog.onSave.subscribe(
                    (data: EmailTemplateData) => {
                        dialogComponent.data.templateId = data.templateId;
                        dialogComponent.onTemplateChanged({
                            value: data.templateId,
                        });
                        dialogComponent.refresh();
                    }
                );
                emailTemplateDialog.onTemplateDelete.subscribe(
                    (templateId: number) => {
        ...
        dialogComponent.onTemplateChange
            .pipe(
                switchMap((templateId: number) => {
                    ...
                    if (onTemplateChange)
                        dataLoader$ = onTemplateChange(
                            templateId,
                            dialogComponent
                        );
                    else {
                        if (data.contactIds)
                            dataLoader$ = this.emailTemplateProxy
                            ...
                            dataLoader$ = this.communicationProxy.getEmailData(
                                templateId,
                                emailData["contactId"]
                            );
                        ...
                        dataLoader$ = dataLoader$.pipe(
                            map((data: GetEmailDataOutput) => {
                                ...
                                dialogComponent.updateTemplateAttachments(
                                    data.attachments
                                );
                            })
                        );
    ...
    showEmailTemplateSelectorDialog(
        templateId,
        templateType: EmailTemplateType,
        saveCallback: (data) => void
    ) {
        let dialogComponent = this.dialog.open(EmailTemplateDialogComponent, {
            panelClass: "slider",
            disableClose: true,
    ...
    sendBulkEmail(
        input: IBulkSendEmailInput,
        finalizeMethod: () => void
    ): Observable<any> {
        return new Observable<any>((observer) => {
            this.communicationProxy
                .bulkEmailSend(new BulkSendEmailInput(input))
                .pipe(finalize(() => finalizeMethod()))
                .subscribe(
                    () => {
                        observer.next();
                    },
    ...
    sendEmail(
        ...
        return new Observable<number>((observer) => {
            this.communicationProxy
                .sendEmail(new SendEmailInput(input))
                .pipe(finalize(() => finalizeMethod()))
                .subscribe(
                    (res) => {
                        observer.next(res);
    ...
    sendSMS(input: ISendSMSInput) {
        return this.communicationProxy
            .sendSMS(new SendSMSInput(input))
    ...
    showInvoiceEmailDialog(invoiceId: number, data: any = {}) {
        ...
        return this.showEmailDialog(
            data,
            "Email",
            (tmpId: number, dialogComponent: EmailTemplateDialogComponent) => {
    ...
    showNoteAddDialog(noteData?: NoteInfoDto) {
        ...
        if (
            dialog &&
            (!noteData || dialog.componentInstance.data.note.id == noteData.id)
        )
            return;
    ...
    showMergeContactDialog(
        ...
        return this.contactProxy
            ...
            .pipe(
                finalize(() => loadFinalize()),
                switchMap((response: GetContactInfoForMergeOutput) => {
                    return this.dialog
                        .open(MergeContactDialogComponent, {
                            panelClass: "slider",
                            disableClose: true,
                            closeOnNavigation: false,
                            data: {
    ...
    showTemplateDocumentsDialog(
        ...
        return this.dialog.open(TemplateDocumentsDialogComponent, {
            ...
            panelClass: ["slider", "crm-document-dialog"],
            hasBackdrop: false,
            closeOnNavigation: true,
    ...
    deleteContact(
        ...
        let canForceDelete = this.permission.isGranted(
            AppPermissions.CRMForceDeleteEntites
        );
        if (isLead) {
            ContactsHelper.showConfirmMessage(
                ...
                (isConfirmed: boolean, [forceDelete]: boolean[]) => {
                    if (isConfirmed) {
                        this.leadService
                            .deleteLead(entityId, forceDelete)
                            .subscribe(() => {
                                abp.notify.success(
                ...
                [
                    {
                        ...
                        visible: canForceDelete,
                        checked: false,
                    },
                ]
            );
        } else {
            let text = contactGroups.some(
                (group) => group == ContactGroup.Partner
            )
                ? this.ls.l("PartnerDeleteWarningMessage", customerName)
                : this.ls.l("ContactDeleteWarningMessage", customerName);
            ContactsHelper.showConfirmMessage(
                text,
                (
                    isConfirmed: boolean,
                    [forceDelete, notifyUser]: boolean[]
                ) => {
                    if (isConfirmed) {
                        this.contactProxy
                            .deleteContact(entityId, forceDelete, notifyUser)
                            .subscribe(() => {
                                abp.notify.success(
                                    this.ls.l("SuccessfullyDeleted")
                                );
    ...
    getSection(queryParams: Params): string {
        if (queryParams) {
            if (queryParams.subId) return "subscriptions";
    ...
    getContactGroupId(queryParams: Params): string {
        if (queryParams) {
            if (queryParams.contactGroupId) {
                ...
                if (
                    Object.keys(ContactGroup).some(
                        (group) => ContactGroup[group] == contactGroupId
                    )
                )
    ...
    updateStatus(
        ...
        return new Observable<any>((observer: Subscriber<any>) => {
            ...
            ContactsHelper.showConfirmMessage(
                this.ls.l(
                    startCase(entity) + "UpdateStatusWarningMessage",
                    this.ls.l((isActive ? "" : "in") + "activate"),
                    startCase(contactGroupName[<string>groupId])
                ),
                (
                    isConfirmed: boolean,
                    [notifyUser, processLead]: boolean[]
                ) => {
                    if (isConfirmed) {
    ...
    private updateStatusInternal(
        ...
        notifyUser: boolean,
        processLead: boolean,
        entityType: "contact" | "user" = "contact"
    ) {
        return entityType === "contact"
            ? this.contactProxy.updateContactStatus(
                  new UpdateContactStatusInput({
                      contactId: entityId,
    ...
    getConfirmedContactText(data: any) {
        ...
        return (
            this.ls.l("ConfirmedContact") +
            (data.isConfirmed && date
                ? " at " +
                  date.format(AppConsts.formatting.dateMoment) +
                  " by " +
                  (data.confirmedByUserFullName || this.ls.l("System"))
                : "")
        );
    }
