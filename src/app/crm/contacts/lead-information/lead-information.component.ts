...
*/
    ...
    constructor(
        ...
        contactsService.orgUnitsSaveSubscribe((data) => {
            ...
            if (
                orgUnitId &&
                orgUnitId != this.data.leadInfo.sourceOrganizationUnitId
            ) {
                ...
                if (contactInfo) {
                    ...
                    this.showApplicationAllowed =
                        this.permissionCheckerService.isGranted(
                            AppPermissions.PFMApplicationsViewApplications
                        ) &&
    ...
    private loadOrganizationUnits() {
        ...
        this.store$
            ...
            .subscribe((organizationUnits: OrganizationUnitShortDto[]) => {
                ...
                this.contactsService.leadInfoSubscribe((leadInfo) => {
                    this.isEditAllowed =
                        leadInfo && leadInfo.id && this.isCGManageAllowed;
                    this.contactsService.orgUnitsUpdate({
            ...
            .pipe(
                finalize(() =>
                    this.loadingService.finishLoading(
                        this.loaderWrapper.nativeElement
                    )
                )
            )
            .subscribe((response) => {
    ...
    updateValue(value, item) {
        ...
        this.leadService
            .updateLeadInfo(UpdateLeadInfoInput.fromJS(this.data.leadInfo))
            .subscribe(
                () => {},
    ...
    formatFieldValue(field, value) {
        if (value instanceof moment) {
            ...
            return this.currencyPipe.transform(
                value,
                this.data.leadInfo.currencyId || this.currency
            );
        else if (field == "ssn")
            ...
            .subscribe(
                (response) => {
                    this.contactsService.orgUnitsUpdate({
                        allOrganizationUnits: this.organizationUnits,
                        selectedOrgUnits: [
                            response.newSourceOrganizationUnitId,
                        ],
                    });
    ...
    toggleOrgUnitsDialog(open: boolean = true): void {
        let dialog = this.dialog.getDialogById(
            "lead-organization-units-dialog"
        );
        if (!dialog) {
            if (open) {
                const dialogData: OrganizationUnitsDialogData = {
                    title:
                        this.ls.l("Toolbar_ReferredBy") +
                        " " +
                        this.ls.l("ReferredByOrgUnit"),
    ...
    openSourceContactList() {
        if (this.isCGManageAllowed) {
            this.sourceComponent.leadId =
                this.data && this.data.leadInfo && this.data.leadInfo.id;
            this.sourceComponent.toggle();
        }
    }
