...
import {
  ...
  LeadServiceProxy,
  LeadInfoDto,
  UpdateLeadSourceOrganizationUnitInput,
  ContactInfoDto,
  ContactServiceProxy,
  UpdateLeadInfoInput,
  OrganizationUnitShortDto,
  UpdateLeadSourceContactInput,
...
export class LeadInformationComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild(SourceContactListComponent)
  sourceComponent: SourceContactListComponent;
  ...
  set selectedTabIndex(val: number) {
    if ((this._selectedTabIndex = val)) this.application || this.loadApplication();
  }
...
*/
      {
        ...
        items: [
          {
            name: 'customField1',
            lname: 'Request_CustomField1',
            readonly: true,
          },
          {
            name: 'customField2',
            lname: 'Request_CustomField2',
            readonly: true,
          },
          {
            name: 'customField3',
            lname: 'Request_CustomField3',
            readonly: true,
          },
          {
            name: 'customField4',
            lname: 'Request_CustomField4',
            readonly: true,
          },
          {
            name: 'customField5',
            lname: 'Request_CustomField5',
            readonly: true,
          },
        ],
      },
    ...
    [
      {
        ...
        items: [
          {
            name: 'affiliateCode',
            lname: 'LeadInformation_SourceAffiliateCode',
          },
        ...
        items: [
          {
            name: 'applicantId',
            readonly: true,
            action: this.showApplications.bind(this),
          },
          {
            name: 'applicationId',
            readonly: true,
  ...
  constructor(
    ...
    contactsService.orgUnitsSaveSubscribe(data => {
      ...
      if (orgUnitId && orgUnitId != this.data.leadInfo.sourceOrganizationUnitId) {
        ...
        leadService
          .updateSourceOrganizationUnit(
            new UpdateLeadSourceOrganizationUnitInput({
  ...
  ngOnInit() {
    this.contactsService.leadInfoSubscribe(leadInfo => {
      if ((this.data.leadInfo = leadInfo)) {
        this.isCGManageAllowed = this.permissionService.checkCGPermission([
    ...
    this.contactsService.contactInfoSubscribe((contactInfo: ContactInfoDto) => {
      if (contactInfo) {
        ...
        this.showApplicationAllowed =
          this.permissionCheckerService.isGranted(AppPermissions.PFMApplicationsViewApplications) &&
          contactInfo.personContactInfo.userId &&
          contactInfo.groups.some(group => group.groupId == ContactGroup.Client);
      ...
      .subscribe((organizationUnits: OrganizationUnitShortDto[]) => {
        ...
        this.contactsService.leadInfoSubscribe(leadInfo => {
          ...
          this.contactsService.orgUnitsUpdate({
            ...
            selectedOrgUnits: leadInfo
              ? [
                  organizationUnits.find((organizationUnit: OrganizationUnitShortDto) => {
                    return (
                      organizationUnit.id ===
                      (leadInfo.sourceOrganizationUnitId ||
                        this.data.leadInfo.sourceOrganizationUnitId)
                    );
                  }),
                ]
                  .filter(Boolean)
  ...
  loadApplication() {
    ...
    this.applicationProxy
      .getInitialMemberApplication(
        this.data.contactInfo.personContactInfo.userId,
        this.data.leadInfo.applicationId
      )
      .pipe(finalize(() => this.loadingService.finishLoading(this.loaderWrapper.nativeElement)))
      .subscribe(response => {
  ...
  updateValue(value, item) {
    ...
    this.leadService.updateLeadInfo(UpdateLeadInfoInput.fromJS(this.data.leadInfo)).subscribe(
      () => {},
  ...
  formatFieldValue(field, value) {
    if (value instanceof moment) {
      ...
      else return value.format(this.formatting.fieldDateTime);
  ...
  hasFieldMoneyType(field) {
    return field == 'netMonthlyIncome' || field.toLowerCase().indexOf('amount') >= 0;
  }
  ...
  onSourceContactChanged(event) {
    if (this.data.leadInfo.sourceContactId == event.id) event = { id: undefined, name: undefined };
    ...
    if (!dialog) {
      if (open) {
        ...
        this.dialog
          ...
          .afterClosed()
          .subscribe(() => this.contactsService.closeSettingsDialog());
      }
  ...
  openSourceContactList() {
    if (this.isCGManageAllowed) {
      this.sourceComponent.leadId = this.data && this.data.leadInfo && this.data.leadInfo.id;
      this.sourceComponent.toggle();
    }
  }
