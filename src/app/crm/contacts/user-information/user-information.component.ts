    ...
    implements OnInit, AfterViewInit, OnDestroy
{
    ...
    @ViewChild("inviteValidationGroup")
    inviteValidationComponent: DxValidationGroupComponent;
    data: any;
    ...
    isEditAllowed = this.permissionService.isGranted(
        AppPermissions.AdministrationUsersEdit
    );
    isInviteAllowed = this.permissionService.isGranted(
        AppPermissions.AdministrationUsersCreate
    );
    changeRolesAllowed = this.permissionService.isGranted(
        AppPermissions.AdministrationUsersChangePermissionsAndRoles
    );
    ...
    showInviteUserForm$: Observable<boolean> =
        this.contactsService.userId$.pipe(
            startWith(null),
            map(
                (userId) =>
                    !userId &&
    ...
    validationRules = {
        ...
        phoneNumber: [
            { type: "stringLength", max: 24 },
            { type: "pattern", pattern: AppConsts.regexPatterns.phone },
        ],
        emailAddress: [
            {
                type: "pattern",
                pattern: this.emailRegEx,
                message: this.ls.l("InvalidEmailAddress"),
            },
            { type: "required", message: this.ls.l("EmailIsRequired") },
        ],
    };
    orgUnitsDisabled;
    dataIsloading = false;
    ...
    ngOnInit() {
        ...
        this.contactsService.userSubscribe((userId: number) => {
            ...
            if ((this.data.userId = userId)) this.loadData();
            else {
            ...
            if (this.data.raw) {
                ...
                this.selectedOrgUnits.forEach((item) => {
                    this.data.raw.memberedOrganizationUnits.push(
                        this.data.raw.allOrganizationUnits.find(
                            (organizationUnit) => {
    ...
    ngAfterViewInit() {
        this.contactsService.settingsDialogOpened$
            .pipe(takeUntil(this.lifecycleSubjectService.destroy$))
            .subscribe((opened) => {
    ...
    private loadOrganizationUnits() {
        this.store$.dispatch(
            new OrganizationUnitsStoreActions.LoadRequestAction(false)
        );
        this.store$
            .pipe(
        ...
        if (this.roles) {
            this.checkedByDefaultRoles = this.isPartner()
                ? this.partnerRoles.filter((partnerRole: AppRoles) => {
                      return (
                          this.roles
                              .map((role: RoleListDto) => role.name)
                              .indexOf(partnerRole) > 0
                      );
                  })
                : [];
    ...
    getPhonesAndEmails() {
        this.contactsService.contactInfo$
            ...
            .subscribe((contactInfo: ContactInfoDto) => {
                this.phones =
                    contactInfo.personContactInfo.details.phones.filter(
                        (item) => item.isActive
                    );
                this.emails =
                    contactInfo.personContactInfo.details.emails.filter(
                        (item) => item.isActive
                    );

                this.inviteData.emailAddress = (
                    this.emails[0] || {}
                ).emailAddress;
                this.inviteData.phoneNumber = (
                    this.phones[0] || {}
                ).phoneNumber;
            });
        ...
        setTimeout(() => {
            let instance =
                this.emailAddressComponent &&
                this.emailAddressComponent.instance;
            if (instance) instance.option("isValid", true);
        });
    }
    ...
    initUserForEdit(contactInfo: ContactInfoDto) {
        if (contactInfo) {
            this.userService
                .getUserForEdit(
                    contactInfo.personContactInfo.userId || undefined
                )
                .pipe(
                    finalize(() => {
    ...
    isPartner() {
        ...
        return (
            contactInfo &&
            contactInfo.hasOwnProperty("groups") &&
            contactInfo.groups.some(
                (group) => group.groupId == ContactGroup.Partner
            )
        );
    }
    ...
    fillUserData(data) {
        ...
        data.memberedOrganizationUnits.forEach(
            (organizationUnitCode: string) => {
                const organizationUnit = data.allOrganizationUnits.find(
                    (organizationUnit) =>
                        organizationUnit.code === organizationUnitCode
                );
                if (organizationUnit) {
        ...
        if (data.user.id) {
            ...
            this.selectedOrgUnits = (
                ...
                .map((item) => {
                    return (
                        item.organization &&
                        item.organization.rootOrganizationUnitId
                    );
                })
                .filter(Boolean);
        }
    ...
    inviteUser() {
        if (
            !this.inviteData.emailAddress ||
            !this.emailAddressComponent.instance.option("isValid")
        )
            return this.message.warn(this.ls.l("InvalidEmailAddress"));

        if (
            this.inviteData.phoneNumber &&
            !this.phoneNumberComponent.instance.option("isValid")
        )
            return this.message.warn(this.ls.l("PhoneValidationError"));

        if (!this.inviteValidationComponent.instance.validate().isValid) return;
        ...
        this.message.confirm(
            ...
            (isConfirmed) => {
                if (isConfirmed) {
                    this.loadingService.startLoading();
                    this.inviteData.contactId =
                        this.contactInfoData.contactInfo.personContactInfo.id;
                    let phoneNumber = this.phoneFormatPipe.transform(
                        this.inviteData.phoneNumber
                    );
                    ...
                    this.personContactServiceProxy
                        ...
                        .subscribe(() => {
                            ...
                            this.appStoreService.dispatchUserAssignmentsActions(
                                Object.keys(ContactGroup),
                                true
                            );
                        });
                }
    ...
    inviteRoleUpdate(event, role) {
        let roles = this.inviteData.assignedRoleNames,
            roleIndex;
        if (event.value) roles.push(role.roleName);
    ...
    updatePhoneNumber(
        countryPhoneNumberComponent: CountryPhoneNumberComponent
    ) {
        if (!countryPhoneNumberComponent.isValid()) return;

        if (countryPhoneNumberComponent.isEmpty())
            this.data.user.phoneNumber = undefined;

        this.update(this.PHONE_FIELD, this.data.user.phoneNumber, () => {
        ...
        if (this.isEditAllowed) {
            this.message.confirm(
                ...
                this.ls.l(
                    ...
                    this.ls.l("Phone").toLowerCase()
                ),
                (result) => {
                    if (!result) return;

                    this.update(this.PHONE_FIELD, undefined, () => {
                        this.data.user.phoneNumber = undefined;
                        this.phoneInplaceEdit = false;
                    });
                }
    ...
    roleUpdate(event, role) {
        ...
        let sub;
        if (role.isAssigned)
            sub = this.userService.addToRole(
                UpdateUserRoleInput.fromJS({
                    id: this.userData.user.id,
                    roleName: role.roleName,
                })
            );
        else
            sub = this.userService.removeFromRole(
            ...
            () => {
                if (
                    this.roles.filter(
                        (item) =>
                            item.id == role.roleId &&
                            item.moduleId == AppConsts.modules.CRMModule
                    ).length
                )
                    this.appStoreService.dispatchUserAssignmentsActions(
                        Object.keys(ContactGroup),
                ...
                .subscribe(
                    (confirm: boolean) => {
                        if (confirm) {
                            ...
                            if (
                                this.data.user.isActive &&
                                contactInfo.groups.every(
                                    (group) => !group.isActive
                                )
                            ) {
                                contactInfo.groups.forEach(
    ...
    update(fieldName?, value?, callback?) {
        ...
        if (fieldName == this.EMAIL_FIELD)
            ...
            [this.LOCKOUT_FIELD, this.TWO_FACTOR_FIELD].indexOf(fieldName) >= 0
        )
            sub = this.userService.updateOptions(
                UpdateUserOptionsDto.fromJS(data)
            );
        else {
            sub = this.userService.createOrUpdateUser(
                CreateOrUpdateUserInput.fromJS({
                    user: this.userData.user,
                    setRandomPassword: this.userData.user["setRandomPassword"],
                    sendActivationEmail:
                        this.userData.user["sendActivationEmail"],
        ...
        sub.pipe(
            ...
            () => {
                ...
                this.notify.info(this.ls.l("SavedSuccessfully"));
                if (
                    [this.EMAIL_FIELD, this.PHONE_FIELD].indexOf(fieldName) >= 0
                )
    ...
    checkInsertCustomValue(list) {
        if (list.indexOf(this.inviteData) < 0) list.unshift(this.inviteData);
    }

    onCustomItemCreating($event, field) {
        let isEmail = field == "emailAddress";
        this.checkInsertCustomValue(isEmail ? this.emails : this.phones);
    ...
    toggleOrgUnitsDialog(open: boolean = true): void {
        const dialog = this.dialog.getDialogById(
            "user-organization-units-dialog"
        );
        if (!dialog) {
            if (open) {
                this.dialog
                    .open(OrganizationUnitsDialogComponent, {
                        id: "user-organization-units-dialog",
                        panelClass: ["slider", "crm-organization-units-dialog"],
                        disableClose: false,
                        hasBackdrop: false,
    ...
    ngOnDestroy() {
        ...
        if (this.dependencyChanged) this.contactsService.invalidate();
