...
@Component({
  ...
  styleUrls: ['../shared/styles/grouped-action-menu.less', './leads.component.less'],
...
export class LeadsComponent extends AppComponentBase implements OnInit, AfterViewInit, OnDestroy {
  ...
  set selectedLeads(leads: LeadDto[]) {
    ...
    leads.forEach((lead: LeadDto) => {
      if (lead && lead.CustomerId) this.selectedClientKeys.push(lead.CustomerId);
    });
  ...
  pipelineEditIsGranted = this.permission.isGranted(AppPermissions.CRMPipelinesConfigure);
  actionEvent: any;
  ...
  pipelines$: Observable<PipelineDto[]> = this.store$.pipe(
    select(
      PipelinesStoreSelectors.getPipelines({
  ...
  selectedPipelineId$: Observable<number> = this._selectedPipelineId
    .asObservable()
  ...
  isPropertyPipeline$: Observable<boolean> = this.selectedPipeline$.pipe(
    map(
      selectedPipeline =>
        selectedPipeline.entityTypeSysId &&
        (selectedPipeline.entityTypeSysId == EntityTypeSys.PropertyAcquisition ||
          selectedPipeline.entityTypeSysId.startsWith(EntityTypeSys.PropertyRentAndSale))
    )
  );
  ...
  manageCGPermission = '';
  sliceStorageKey =
    'CRM_Contacts_Slice_' + this.sessionService.tenantId + '_' + this.sessionService.userId;
  ...
  showCompactView$: Observable<Boolean> = combineLatest(
    ...
    map(
      ([layoutType, pipelineCompactView, gridCompactView]: [DataLayoutType, Boolean, Boolean]) => {
        return layoutType == DataLayoutType.Pipeline ? pipelineCompactView : gridCompactView;
      }
    )
  );
  hidePipeline$: Observable<boolean> = this.dataLayoutType$.pipe(
    map((dataLayoutType: DataLayoutType) => {
      return dataLayoutType !== DataLayoutType.Pipeline;
    })
  );
  hideDataGrid$: Observable<boolean> = this.dataLayoutType$.pipe(
    map((dataLayoutType: DataLayoutType) => {
      return dataLayoutType !== DataLayoutType.DataGrid;
    })
  );
  hidePivotGrid$: Observable<boolean> = this.dataLayoutType$.pipe(
    map((dataLayoutType: DataLayoutType) => {
      return dataLayoutType !== DataLayoutType.PivotGrid;
    })
  );
  hideChart$: Observable<boolean> = this.dataLayoutType$.pipe(
    map((dataLayoutType: DataLayoutType) => {
      return dataLayoutType !== DataLayoutType.Chart;
    })
  );
  hideMap$: Observable<boolean> = this.dataLayoutType$.pipe(
    map((dataLayoutType: DataLayoutType) => {
  ...
  public headlineButtons$: Observable<HeadlineButton[]> = this.selectedPipeline$.pipe(
    map((selectedPipeline: PipelineDto) => {
      ...
      localizedLabel =
        this.l('Pipeline_' + selectedPipeline.name + '_Single') !== localizedLabel
          ? localizedLabel
  ...
  private pivotGridDataSource = {
    ...
    load: loadOptions => {
      ...
      return this.selectedContactGroup$
        .pipe(
          first(),
  ...
  chartDataSource = new DataSource({
    key: 'id',
    load: () => {
      return this.odataRequestValues$
        .pipe(
          first(),
          withLatestFrom(this.selectedContactGroup$),
          switchMap(
            ([odataRequestValues, selectedContactGroup]: [ODataRequestValues, ContactGroup]) => {
              const chartDataUrl =
                this.chartDataUrl ||
                this.crmService.getChartDataUrl(
                  this.getODataUrl(this.groupDataSourceURI),
                  odataRequestValues,
  ...
  maxMessageCount = this.contactService.getFeatureCount(
  ...
  constructor(
    ...
    this.crmService.updateCountryStateFilter(
    ...
    this.selectedPipelineId$.pipe(first()).subscribe((selectedPipelineId: number) => {
      ...
      this.totalDataSource = new DataSource({
        ...
        store: new ODataStore({
          ...
          onLoaded: (count: any) => {
            let dataSource = this.showPipeline ? this.pipelineDataSource : this.dataSource;
            if (!isNaN(count)) dataSource['total'] = this.totalCount = count;
          },
          errorHandler: (e: any) => {
      ...
      ).pipe(filter((odataRequestValues: ODataRequestValues) => !!odataRequestValues));
  ...
  private initContactGroupRelatedProperties() {
    ...
    if (this.isSmsAndEmailSendingAllowed) this.pipelineSelectFields.push(this.leadFields.Phone);
    ...
    this.actionMenuGroups = [
      {
        ...
        items: [
          ...
          },
          {
            ...
            checkVisible: (lead: LeadDto) => {
              return this.permission.checkCGPermission(
                [this.selectedContactGroup],
                'ViewCommunicationHistory.SendSMSAndEmail'
              );
            },
          ...
          },
          {
            getText: (lead: LeadDto) => {
              ...
              return (
                this.l('Checklist') +
                ' (' +
                lead.StageChecklistPointDoneCount +
                '/' +
                stage.checklistPoints.length +
                ')'
              );
            },
            class: 'checklist',
        ...
        items: [
          {
            ...
            checkVisible: (lead: LeadDto) =>
              this.permission.checkCGPermission([this.selectedContactGroup]),
          },
          {
            text: this.l('EditRow'),
            class: 'edit',
  ...
  private handleUserGroupTextUpdate() {
    this.userGroupText$
      .pipe(takeUntil(this.lifeCycleSubjectsService.destroy$))
  ...
  private setInitialPipelineId() {
    this.pipelines$.subscribe((pipelines: PipelineDto[]) => {
      ...
      let pipelineId =
        this._activatedRoute.snapshot.queryParams.pipelineId ||
        this.cacheService.get(this.cacheKey);
  ...
  private handleFiltersPining() {
    ...
    filterFixed$.pipe(switchMap(this.waitUntil(DataLayoutType.DataGrid))).subscribe(() => {
      this.repaintDataGrid(1000);
    });
    filterFixed$.pipe(switchMap(this.waitUntil(DataLayoutType.PivotGrid))).subscribe(() => {
  ...
  private handleTotalCountUpdate() {
    combineLatest(this.odataRequestValues$, this.refresh$)
      .pipe(
        debounceTime(300),
        filter(() => !this.showPipeline),
        takeUntil(this.lifeCycleSubjectsService.destroy$),
        switchMap(([odataRequestValues]: [ODataRequestValues, null]) => {
          return this.loadTotalsRequest$.pipe(
            first(),
            map(() => odataRequestValues)
          );
        })
      )
  ...
  private handleLayoutDataUpdate() {
    [DataLayoutType.PivotGrid, DataLayoutType.Pipeline].forEach(layout => {
      this.listenForUpdate(layout)
  ...
  private handleDataGridUpdate() {
    this.listenForUpdate(DataLayoutType.DataGrid)
      .pipe(skip(1), debounceTime(300))
      .subscribe(() => {
        if (this.isDataLoaded) this.processFilterInternal();
      });
  }

  private handleChartUpdate() {
    combineLatest(this.chartComponent.summaryBy$, this.listenForUpdate(DataLayoutType.Chart))
      .pipe(takeUntil(this.lifeCycleSubjectsService.destroy$))
      .subscribe(
        ([summaryBy, [odataRequestValues, contactGroup]]: [
          SummaryBy,
          [ODataRequestValues, ContactGroup]
        ]) => {
  ...
  private handleModuleChange() {
    merge(this.dataLayoutType$, this.activate$.pipe(filter(Boolean)))
      .pipe(takeUntil(this.destroy$))
      .subscribe(() => {
  ...
  private handleFilterParams() {
    this.queryParams$
      .pipe(
        ...
        switchMap((queryParams: Params) => this.activate$.pipe(filter(Boolean), mapTo(queryParams)))
      )
    ...
    const queryDataLayoutType$ = this.queryParams$.pipe(
      ...
      filter(
        (dataLayoutType: DataLayoutType) =>
          dataLayoutType && dataLayoutType != this.dataLayoutType.value
      )
    );
  ...
  private listenForUpdate(layoutType: DataLayoutType) {
    return combineLatest(this.odataRequestValues$, this.selectedContactGroup$, this.refresh$).pipe(
      takeUntil(this.lifeCycleSubjectsService.destroy$),
  ...
  private waitUntil(layoutType: DataLayoutType) {
    return data =>
      this.dataLayoutType.value == layoutType
        ? of(data)
        : this.dataLayoutType$.pipe(
  ...
  private getContactsData(): Observable<any> {
    return combineLatest(this.listenForUpdate(DataLayoutType.Map), this.selectedMapArea$).pipe(
      map(
        ([[oDataRequestValues, contactGroupId], mapArea]: [
          [ODataRequestValues, ContactGroup, null],
          MapArea
        ]) => {
  ...
  getOrganizationUnitName = e => {
    return DataGridService.getOrganizationUnitName(
      e.SourceOrganizationUnitId,
  ...
  onContentReady(event) {
    if (this.exportCallback) this.exportCallback();
    else {
      if (this.showDataGrid || this.showGallery) this.setGridDataLoaded();
      event.component.columnOption('command:edit', {
        visibleIndex: -1,
        width: 40,
      });
    }
    if (!this.rowsViewHeight) this.rowsViewHeight = DataGridService.getDataGridRowsViewHeight();

    setTimeout(() => {
      this.appService.isClientSearchDisabled = this.dataLayoutType.value == DataLayoutType.Pipeline;
    });
  }
        ...
        (this.filters = [
                ...
                dataSource$: this.selectedPipelineId$.pipe(
                  switchMap((selectedPipelineId: number) => {
                    return this.store$.pipe(
                      select(
                        PipelinesStoreSelectors.getPipelineTreeSource({ id: selectedPipelineId })
                      )
                    );
                  })
                ),
          ...
          (this.filterModelStar = new FilterModel({
            ...
            items: {
              element: new FilterCheckBoxesModel({
                dataSource$: this.store$.pipe(
                  select(StarsStoreSelectors.getStars),
                  tap(stars => {
                    stars &&
                      stars.forEach(star => {
  ...
  initToolbarConfig() {
    if (this.selectedContactGroup) {
      ...
      this.manageCGPermission = this.permission.getCGPermissionKey(
        [this.selectedContactGroup],
        'Manage'
      );
    }
        ...
        items: [
          {
            ...
            options: {
              ...
              mouseout: () => {
                if (!this.filtersService.fixed) this.filtersService.disable();
              },
            },
            attr: {
        ...
        items: [
          {
            ...
            attr: {
              'filter-selected':
                this.filterModelAssignment && this.filterModelAssignment.isSelected,
              class: 'assign-to',
            },
          },
          {
            ...
            attr: {
              'filter-selected':
                !!this.filterModelSource.items.element['contact'] ||
            ...
            options: {
              items: [
                ...
                },
                {
                  ...
                  visible:
                    this.appSession.isPerformancePartnerTenant &&
                    this.selectedContactGroup == ContactGroup.Partner,
                  action: () => {
                    this.message.confirm(
                      '',
                      this.l(
                        'ReferralPartnersSendEmailConfirmation',
                        this.selectedClientKeys.length
                      ),
                      res => {
                        if (res) {
        ...
        items: [
          {
            ...
            options: {
              ...
              items: [
                  ...
                  action: this.exportData.bind(this, options => {
                    if (this.showPivotGrid) {
                      this.pivotGridComponent.dataGrid.instance.option(
                        'export.fileName',
                        this.exportService.getFileName(null, 'PivotGrid', this.userGroupText)
                      );
                      this.pivotGridComponent.dataGrid.instance.exportToExcel();
                    } else if (this.showPipeline || this.showDataGrid || this.showGallery) {
                      return this.exportToXLS(options, null, this.userGroupText);
                    }
                  }),
                ...
                {
                  action: this.exportData.bind(this, options =>
                    this.exportToCSV(options, null, this.userGroupText)
                  ),
                ...
                {
                  action: this.exportData.bind(this, options =>
                    this.exportToGoogleSheet(options, null, this.userGroupText)
                  ),
        ...
        items: [
          {
            ...
            options: {
              items: [
                { text: 'Old Pipeline', icon: 'undo' },
                { text: 'New Pipeline', icon: 'redo' },
              ],
              displayExpr: 'text',
              keyExpr: 'text',
              onItemClick: e => {
                this.toggleDataLayout(DataLayoutType.Pipeline);
                this.showNewPipeline = e.itemData.text === 'New Pipeline';
              },
              text: !this.showNewPipeline ? 'New Pipeline' : 'Old Pipeline',
            },
          },
          {
            name: 'pipeline',
    ...
    if (this.showPipeline) {
      let importOption = 'all',
        ...
        checkExportOption = (dataSource, ignoreFilter: boolean = false) => {
          if (options == importOption) ignoreFilter || this.processFilterInternal([this]);
          else if (!this.exportPipelineSelectedItemsFilter(dataSource)) importOption = options;
        };

      if (dataSource) {
        checkExportOption(dataSource, true);
        callback(importOption).then(() => dataSource.filter(null));
      } else {
  ...
  toggleCompactView() {
    if (this.showPipeline) this.pipelineService.toggleContactView();
    else {
    ...
    if (this.showPipeline || this.showDataGrid || this.showPivotGrid || this.showGallery) {
      contexts =
        contexts && contexts.length
          ? contexts
          : [this.showPipeline ? this.pipelineComponent : this];
      contexts.forEach(context => {
        if (context && context.processODataFilter) {
          const dataGridInstance = this.showPivotGrid
            ? this.pivotGridComponent &&
              this.pivotGridComponent.dataGrid &&
              this.pivotGridComponent.dataGrid.instance
          ...
          if (this.showPipeline || dataGridInstance) {
            ...
            if (this.dataSource) this.dataSource['total'] = this.dataSource['entities'] = undefined;
  ...
  private setDataGridInstance() {
    ...
    if (instance && !instance.option('dataSource')) {
      this.oDataService
        .getODataFilter(this.filters, this.filtersService.getCheckCustom)
        .subscribe((odataRequestValues: ODataRequestValues) => {
      ...
      if (!instance.option('paging.pageSize')) instance.option('paging.pageSize', 20);
  ...
  private setChartInstance() {
    const chartInstance =
      this.chartComponent && this.chartComponent.chart && this.chartComponent.chart.instance;
  ...
  updateLeadsStage($event) {
    if (this.hasBulkPermission) {
      ...
      this.pipelineService
        .updateEntitiesStage(
          this.pipelinePurposeId,
          this.selectedLeads.filter(lead => lead.Stage != $event.name),
          $event.name,
          this.selectedContactGroup,
          this.selectedPipelineId
        )
          ...
          if (this.showDataGrid || this.showGallery) {
            ...
            if (gridInstance && declinedList && declinedList.length)
              gridInstance.selectRows(
                declinedList.map((item: LeadDto) => item.Id),
                false
              );
            else gridInstance.clearSelection();
          } else {
            this._refresh.next(null);
          }
  ...
  showLeadDetails(event, section?: string, queryParams?: Params) {
    ...
    this.selectedPipeline$.pipe(first()).subscribe((selectedPipeline: PipelineDto) => {
      ...
      setTimeout(() => {
        this._router.navigate(CrmService.getEntityDetailsLink(clientId, section, leadId, orgId), {
          queryParams: {
  ...
  deleteLeads(leadIds?: number[]) {
    ...
    ContactsHelper.showConfirmMessage(
      ...
      [
        {
          text: this.l('ForceDelete'),
          visible: this.permission.isGranted(AppPermissions.CRMForceDeleteEntites),
          checked: false,
        },
      ]
    );
  ...
  editPipeline({ id, value }) {
    if (this.pipelineEditIsGranted)
      this.pipelineServiceProxy
        .rename(
          new PipelineRenameInput({
            id: id,
            name: value,
          })
        )
  ...
  onOwnerFilterApply(event?) {
    ...
    this.filterModelOrgUnit.items.element.value =
      filter && (!event || filter[0] == event.id) ? [] : [event.id];
  ...
  onSourceApply(contacts) {
    if (this.isGranted(AppPermissions.CRMBulkUpdates)) {
      ContactsHelper.showConfirmMessage(
        this.l(
          'UpdateForSelected',
          this.l('SourceContactName'),
          this.l('ContactGroup_' + this.contactGroupNames[String(this.selectedContactGroup)])
        ),
        (confirmed: boolean, [applyCode]: boolean[]) => {
          if (confirmed)
            this.leadService
              .updateSourceContacts(
                new UpdateLeadSourceContactsInput({
              ...
              .subscribe(() => {
                this.selectedLeads = [];
                if (this.dataGrid && this.dataGrid.instance) this.dataGrid.instance.deselectAll();
                this.processFilterInternal();
