...
export class ImportLeadsComponent extends AppComponentBase implements AfterViewInit, OnDestroy {
  ...
  private readonly BUSINESS_COMPANY_FULL_ADDRESS_NEIGHBORHOOD =
    'businessInfo_companyFullAddress_neighborhood';
  private readonly BUSINESS_COMPANY_FULL_ADDRESS_ADDRESSLINE2 =
    'businessInfo_companyFullAddress_addressline2';
  private readonly BUSINESS_COMPANY_FULL_ADDRESS_CITY = 'businessInfo_companyFullAddress_city';
  private readonly BUSINESS_COMPANY_FULL_ADDRESS_STATE_NAME =
    'businessInfo_companyFullAddress_stateName';
  private readonly BUSINESS_COMPANY_FULL_ADDRESS_STATE_CODE =
    'businessInfo_companyFullAddress_stateId';
  private readonly BUSINESS_COMPANY_FULL_ADDRESS_ZIP_CODE = 'businessInfo_companyFullAddress_zip';
  private readonly BUSINESS_COMPANY_FULL_ADDRESS_COUNTRY_NAME =
    'businessInfo_companyFullAddress_countryName';
  private readonly BUSINESS_COMPANY_FULL_ADDRESS_COUNTRY_CODE =
    'businessInfo_companyFullAddress_countryId';
  ...
  private readonly BUSINESS_WORK_FULL_ADDRESS_NEIGHBORHOOD =
    'businessInfo_workFullAddress_neighborhood';
  private readonly BUSINESS_WORK_FULL_ADDRESS_ADDRESSLINE2 =
    'businessInfo_workFullAddress_addressline2';
  ...
  private readonly BUSINESS_WORK_FULL_ADDRESS_COUNTRY_NAME =
    'businessInfo_workFullAddress_countryName';
  private readonly BUSINESS_WORK_FULL_ADDRESS_COUNTRY_CODE =
    'businessInfo_workFullAddress_countryId';
  ...
  private readonly FIELDS_DATE_TIME = [this.DATE_CREATED, this.FOLLOW_UP_DATE];
  ...
  private readonly FIELDS_TO_IGNORE = [this.PERSONAL_CREDITSCORERATING];
  ...
  readonly compareFields: any = [
    ...
    [
      this.PERSONAL_FULL_ADDRESS + ':' + this.LAST_NAME_FIELD,
      this.BUSINESS_COMPANY_FULL_ADDRESS + ':' + this.LAST_NAME_FIELD,
      this.BUSINESS_WORK_FULL_ADDRESS + ':' + this.LAST_NAME_FIELD,
    ],
    [
      this.PERSONAL_FULL_ADDRESS2 + ':' + this.LAST_NAME_FIELD,
      this.BUSINESS_COMPANY_FULL_ADDRESS + ':' + this.LAST_NAME_FIELD,
      this.BUSINESS_WORK_FULL_ADDRESS + ':' + this.LAST_NAME_FIELD,
    ],
    [
      this.PERSONAL_FULL_ADDRESS3 + ':' + this.LAST_NAME_FIELD,
      this.BUSINESS_COMPANY_FULL_ADDRESS + ':' + this.LAST_NAME_FIELD,
      this.BUSINESS_WORK_FULL_ADDRESS + ':' + this.LAST_NAME_FIELD,
    ],
  ];
  ...
  private initFieldsConfig() {
    ...
    this.FIELDS_ORDER.forEach(field => {
      ...
      else this.fieldsConfig[field] = { visibleIndex: fieldIndex };
      fieldIndex++;
    });
    ...
    this.FIELDS_CAPTIONS.forEach(field => {
      ...
      if (
        field.indexOf(this.PERSONAL_FULL_ADDRESS) < 0 &&
        ...
        caption =
          this.l(
            ImportWizardComponent.getFieldLocalizationName(
              parts.join(ImportWizardComponent.FieldSeparator)
            )
          )
            .split(' ')
            .shift() +
      ...
      if (this.fieldsConfig[field]) this.fieldsConfig[field].caption = caption;
      else this.fieldsConfig[field] = { caption: caption };
    });
  }
      ...
      this.setFieldsIfDefined(
        {
          ...
          [field.mappedField + '_street']: [
            ...
            .filter(Boolean)
            .join(' '),
          [field.mappedField + '_addressLine2']: [parsed.sec_unit_type, parsed.sec_unit_num]
            .filter(Boolean)
            .join(' '),
        },
        dataSource
  ...
  private normalizeGenderValue(field, value, dataSource) {
    value = value.trim().toLowerCase();
    if (['f', 'female', '0'].indexOf(value) >= 0) dataSource[field.mappedField] = 'Female';
    else if (['m', 'male', '1'].indexOf(value) >= 0) dataSource[field.mappedField] = 'Male';
    else return false;
    return true;
  }
      ...
      isConfirmed => {
        if (isConfirmed) {
          ...
          if (requestSize > this.MAX_REQUEST_SIZE) {
            ...
            this.sendData(JSON.stringify(leadsInput))
              ...
              .subscribe(
                (importId: number) => {
                  if (importId && !isNaN(importId))
                    setTimeout(() => {
                      this.importProxy
                        .getStatuses(importId)
                        .subscribe((res: GetImportStatusOutput[]) => {
                          let importStatus: GetImportStatusOutput = res[0];
                          this.updateImportStatus(importStatus);
                          if (!this.showedFinishStep()) this.wizard.showFinishStep();
  ...
  sendData(payload: string) {
    return new Observable(subscriber => {
      ...
      xhr.addEventListener('load', () => {
        ...
        if (xhr.status === 200) subscriber.next(response.result);
        else subscriber.error(response.error);
        subscriber.complete();
      });
  ...
  createLeadsInput(data: any): ImportInput {
    let result = ImportInput.fromJS({
      ...
      partnerTypeName:
        this.importType === ImportTypeInput.Partner ? this.selectedPartnerTypeName : undefined,
      ...
      keys.forEach(key => {
        ...
        if (path.length) {
          ...
          currentObj[path[path.length - 1]] =
            this.FIELDS_ARRAY.indexOf(key) >= 0 ? row[key].split(',') : row[key];
        }
      });
    ...
    return (
      path
        .map((item, index) =>
          this.l(
            ImportWizardComponent.getFieldLocalizationName(
              path.slice(0, index + 1).join(ImportWizardComponent.FieldSeparator)
            )
          )
        )
    ...
    keys.forEach(key => {
      ...
      if (this.importType != ImportTypeInput.Client || key != 'leadStageName') {
        if (this.mappingObjectNames[key]) {
          ...
          if (this.FIELDS_TO_IGNORE.indexOf(combinedName) > -1) return;
  ...
  getStages() {
    this.pipelineService
      .getPipelineDefinitionObservable(this.pipelinePurposeId, this.contactGroupId)
      .subscribe(result => {
  ...
  activate() {
    ...
    this.getStages();
    if (this.showedFinishStep()) setTimeout(() => this.reset());
  }
  ...
  checkZipUSCountryFields(field, data, zipField, countryCodeField, countryNameField): boolean {
    if (field.mappedField === zipField) {
      ...
      if (data[countryCodeField]) return data[countryCodeField] == Country.USA;
      ...
      } else return isUSDefaultCountry;
    }
    return false;
  }
  ...
  preProcessFieldBeforeReview = (field, sourceValue, reviewDataSource) => {
    if (field.mappedField == this.FULL_NAME_FIELD) {
      ...
      field.mappedField == this.PERSONAL_FULL_ADDRESS ||
      ...
      if (moment(sourceValue).toJSON())
        reviewDataSource[field.mappedField] = DateHelper.removeTimezoneOffset(
  ...
  validateFieldsMapping = rows => {
    ...
    result.isMapped = rows.every(row => {
      ...
      isCompanyName =
        isCompanyName || (row.mappedField && row.mappedField == this.COMPANY_NAME_FIELD);
  ...
  private initToolbarConfig() {
    ...
    let disabledManage =
      !this.contactGroupId || !this.permission.checkCGPermission([this.contactGroupId]);
    this.toolbarConfig = [
      {
        location: 'before',
        locateInMenu: 'auto',
        items: [
          {
            ...
            options: {
              ...
              items: Object.keys(ImportTypeInput).map((importType: ImportTypeInput) => {
                const text = this.l(importType + 's');
                const contactGroup =
                  this.importLeadsService.getContactGroupFromInputType(importType);
                return {
                  disabled:
                    importType == ImportTypeInput.Order ||
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            attr: {
              'filter-selected':
                this.importType === ImportTypeInput.Partner && !!this.selectedPartnerTypeName,
            },
  ...
  onStepChanged(event) {
    if (event) this.hideLeftMenu = event.selectedIndex != this.wizard.UPLOAD_STEP_INDEX;
    else setTimeout(() => this.initToolbarConfig());
  }
  ...
  getUserAssignmentPermissionKey() {
    return this.contactGroupId
      ? this.permission.getCGPermissionKey([this.contactGroupId], 'ManageAssignments')
      : '';
  }
}
