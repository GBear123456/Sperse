...
/** Application imports */
import { CustomerListsServiceProxy, CustomerListInfoDto, AddCustomersToListsInput, UpdateCustomerListInput, UpdateCustomerListsInput } from '@shared/service-proxies/service-proxies';
...
export class ListsStoreEffects {
    constructor(private _listsService: CustomerListsServiceProxy,
    ...
    addListRequest$: Observable<Action> = this.actions$.pipe(
        ...
        switchMap(payload => {
            ...
            if (payload.serviceMethodName === 'addCustomersToLists') {
                request = this._listsService[payload.serviceMethodName ](AddCustomersToListsInput.fromJS({
                             customerIds: payload.customersIds,
                             lists: payload.lists
                          }));
            } else if (payload.serviceMethodName === 'updateCustomersTags') {
                request = this._listsService.updateCustomerLists(UpdateCustomerListsInput.fromJS({
    ...
    renameListRequest$: Observable<Action> = this.actions$.pipe(
        ...
        map(action => action.payload),
        switchMap(payload => {
            return this._listsService.rename(UpdateCustomerListInput.fromJS({
                id: payload.id,
                name: payload.name
            })).pipe(
                finalize(() => {
