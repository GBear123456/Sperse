...
export class PipelineComponent extends AppComponentBase implements OnInit, OnDestroy {
  ...
  get isChecklistAllowed(): boolean {
    return (
      !!this.appService.getFeatureCount(AppFeatures.CRMMaxChecklistPointCount) &&
      [AppConsts.PipelinePurposeIds.lead, AppConsts.PipelinePurposeIds.order].indexOf(
        this.pipeline.purpose
      ) >= 0
    );
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      (changes.pipelineId &&
        !changes.pipelineId.firstChange &&
        changes.pipelineId.currentValue !== changes.pipelineId.previousValue) ||
      (changes.contactGroupId &&
        !changes.contactGroupId.firstChange &&
  ...
  getEntityName(entity: any): string {
    ...
    if (entity) {
      ...
      entityName =
        prioritizedName || entity.Title || entity.Email || entity.Phone || entity.CompanyName;
    }
    return entityName;
  }
  ...
  getEntitySubtitle(entity: any): string {
    let subtitle: string;
    if (entity) {
      let prioritizedSubtitle = this.currencyPipe.transform(
        entity.Amount,
        entity.CurrencyId || this.currency
      );
      if (
        this.pipeline.entityTypeSysId &&
        this.pipeline.entityTypeSysId.startsWith(EntityTypeSys.PropertyRentAndSale)
      ) {
        ...
        prioritizedSubtitle =
          entity.Name || entity.Title || entity.Email || entity.Phone
            ? prioritizedSubtitle || entity.CompanyName
            : '';
      }
  ...
  private handleDataSource() {
    this.subscribers.push(
      this.pipelineService.dataLayoutType$
        .pipe(
          filter(
            () =>
              !this.pipeline ||
              this.pipeline.contactGroupId != this.contactGroupId ||
        ...
        .subscribe((pipeline: PipelineDto) => {
          ...
          this.stages = pipeline.stages.map((stage: Stage) => {
            ...
            return new Stage({
              ...
              width:
                columnWidthsCache && columnWidthsCache[stage.id]
  ...
  private handleDragAndDrop() {
    this.subscribers.push(
      this.dragulaService.drop(this.dragulaName).subscribe(value => {
        ...
        if (value && value.name == this.dragulaName) {
          ...
          if (value.el.classList.contains('selected')) {
              ...
              if (entity) {
                ...
                this.updateEntityStage(
                  ...
                  (cancelled: boolean) => {
                    entity.selected = false;
                    if (cancelled) {
                      newStage.isLoading = oldStage.isLoading = false;
                      this.detectChanges();
                    } else {
                      this.onEntityStageChanged && this.onEntityStageChanged.emit(entity);
                      let entities = oldStage.entities,
                        itemIndex = entities.indexOf(entity);
                      if (entity.Id != entityId && itemIndex >= 0) entities.splice(itemIndex, 1);
                      reloadStageList.push(oldStage.stageIndex);
            ...
            if (!stage.entities.length) reloadStageList.push(stage.stageIndex);

            this.updateEntityStage(targetEntity, newStage, stage, (cancelled: boolean) => {
              if (cancelled) {
                stage.isLoading = newStage.isLoading = false;
                this.detectChanges();
              } else {
                this.reloadStagesInternal([stage.stageIndex, newStage.stageIndex])
                  .pipe(finalize(() => (stage.isLoading = false)))
                  .subscribe(() => this.detectChanges());
                this.onEntityStageChanged &&
                  this.onEntityStageChanged.emit(this.getEntityById(entityId, newStage));
              }
            });
          }
        }
    ...
    this.subscribers.push(
      this.dragulaService.dragend(this.dragulaName).subscribe(value => {
        if (value.name == this.dragulaName) this.hideStageHighlighting();
      })
    );
    ...
    this.dragulaService.createGroup(this.dragulaName, {
      ...
      moves: (el, source) => {
        if (this.moveDisabled || this.internalMoveDisabled) return false;
          ...
          if (stage)
            stage.accessibleActions.forEach(action => {
              if (action.targetStageId) {
                ...
                const targetElm: HTMLElement = document.querySelector(
                  '[accessKey="' + target.id + '"]'
                );
                if (targetElm) {
      ...
      accepts: (el, target, source) => {
        ...
        if (stageSource && stageTarget) {
          return (
            stageSource.id == stageTarget.id ||
            stageSource.accessibleActions.some(action => {
              return action.targetStageId == stageTarget.id;
            })
          );
        } else return false; // elements can't be dropped in any of the `containers` by default
      },
    });
  }
  ...
  private handleContactView() {
    this.subscribers.push(
      this.pipelineService.compactView$
        ...
        .subscribe((compactView: boolean) => {
          this.compactView = compactView;
          this.stagePageCount = compactView
            ? this.COMPACT_VIEW_PAGE_COUNT
            : this.DEFAULT_PAGE_COUNT;
          this.detectChanges();
        })
    );
    this.subscribers.push(
      this.pipelineService.compactView$
        .pipe(
          ...
          filter(
            (stage: Stage) => !stage.isFull && stage.entities.length < this.COMPACT_VIEW_PAGE_COUNT
          ),
          /** Map to stages ids */
    ...
    if (!this.refreshTimeout) {
      this.refreshTimeout = setTimeout(() => {
        if (this.pipeline) {
          ...
          this.store$.dispatch(
            new PipelinesStoreActions.LoadRequestAction(skipAlreadyLoadedChecking)
          );
      });
    }
  }
  ...
  private checkFilterExcludeCondition(stageId) {
    return (
      ...
      this.filterModelStages.isSelected &&
  ...
  private loadStagesEntities(
    page = 0,
    stageIndex?: number,
    oneStageOnly = false,
    skipTotalRequest = false
  ): Observable<any> {
    let response = of([]),
      index = stageIndex || 0,
    ...
    if (this.checkFilterExcludeCondition(stage.id)) {
      ...
      dataSource.pageSize(
        Math.max((!page && stage.entities && stage.entities.length) || 0, this.stagePageCount)
      );
      dataSource.sort([
      ...
      const odataUrl = this.getODataUrl(
        ...
        this.queryWithSearch
          .filter(item => {
            return (
              !item.hasOwnProperty('or') ||
              item.or.every(
                filter => typeof filter != 'string' || !filter.includes(stage.id.toString())
              )
            );
          })
          .concat({
            and: [
              extend(filter, this._dataSource.customFilter && this._dataSource.customFilter.odata),
            ],
          }),
        null,
        this.params
      ...
      if (!this.odataService.requestLengthIsValid(odataUrl)) {
        ...
        response = from(this.odataService.loadDataSource(dataSource, contextKey, odataUrl)).pipe(
          finalize(() => {
            this.detectChanges();
          ...
          map((entities: any) => {
            if (entities.length) {
              stage.entities = (
                ...
                stage.lastStageIndex = Math.min(
                  (page ? stage.lastStageIndex : undefined) || Infinity,
                  entity.SortOrder
                );
                return entity;
              });
              if (!this.totalsURI) stage.total = dataSource.totalCount();
              stage.isFull = stage.entities.length >= (stage.total || 0);
            } else {
              if (!page || !stage.entities) stage.entities = [];
      ...
      response.subscribe(
        ...
        error => {
          stage.isLoading = false;
          if (error != 'canceled') this.message.error(error);
        }
      );
      this.detectChanges();
    }
  ...
  private getTotalsRequestUrl(filter) {
    ...
    if (customFilter && customFilter.odata) filters = filters.concat({ and: [customFilter.odata] });

    if (customFilter && customFilter.params) params = [...this.params, ...customFilter.params];
    return this.getODataUrl(this.totalsURI, filters, null, params);
  }
  ...
  private processTotalsRequest(filter?: any) {
    if (!this._totalDataSource) {
      ...
      this._totalDataSource['_store']['_requestDispatcher']['_url'] =
        this.getTotalsRequestUrl(filter);
    }

    if (!this._totalDataSource.isLoading()) {
      this.odataService.loadDataSource(this._totalDataSource, this.totalsURI).then(
        res => {
          if (res && res.length) {
            let stages = res.pop();
  ...
  private getBeforeSendEvent(context?) {
    return request => {
      if (context) request.headers['context'] = context;
  ...
  private getDataSourceForStage(stage) {
    let config = cloneDeep(this._dataSource),
      ...
      selectFields = this.selectFields.concat(
        ['SortOrder', this.isChecklistAllowed ? 'StageChecklistPointDoneCount' : null].filter(
          Boolean
        )
      );
  ...
  private reloadStagesInternal(stageIndexList: number[]): Observable<any> {
    return forkJoin.apply(
      this,
      stageIndexList
        .filter((val, index) => stageIndexList.indexOf(val) == index)
        .map(stageIndex => this.loadData(0, stageIndex, true))
    );
  }

  private updateEntityStage(
    entity,
    newStage: Stage,
    oldStage: Stage,
    complete = null,
    ...
    if (entity && entity.Id) {
      setTimeout(() => {
        ...
        if (newStage.name != oldStage.name) {
          this.pipelineService.updateEntityStage(entity, oldStage, newStage, complete, forced);
        } else this.pipelineService.updateEntitySortOrder(this.pipeline.id, entity, complete);
        this.detectChanges();
      });
    }
  }
  ...
  getDateWithTimezone(utcDateTime) {
    if (utcDateTime) return new Date(moment(utcDateTime).format('YYYY/MM/DD HH:mm:ss'));
  }
  ...
  private toogleHighlightShiftArea(entity, checked: boolean) {
    if (this.shiftStartEntity && this.shiftStartEntity.StageId == entity.StageId) {
      const stage = this.getStageById(entity.StageId);
      let startEntityIndex: any = stage.entities.findIndex(
        entity => entity.Id === this.shiftStartEntity.Id
      );
    ...
    } else this.shiftStartEntity = entity;
  }
  ...
  private onKeyUp(event) {
    if (event.keyCode == 16 /*SHIFT*/) this.shiftStartEntity = null;
  }
  ...
  onCardClickInternal(entity, event) {
    ...
    if (event.ctrlKey || event.shiftKey || clickedOnCheckbox) {
      ...
      if (!entity.selected && event.ctrlKey && event.shiftKey) this.deselectAllCards();
      else if (event.shiftKey) this.toogleHighlightShiftArea(entity, entity.selected);
      this.selectedEntities = this.getSelectedEntities();
    } else {
      this.onCardClick.emit({
        entity: entity,
        entityStageDataSource: this._dataSources[entity.Stage],
        loadMethod: this.loadMore.bind(
          this,
          this.stages.findIndex(stage => stage.id === entity.StageId)
        ),
      });
    }
  ...
  updateStage(stage: Stage, actionType: string) {
    ...
    this.dialog
      .open(AddRenameMergeDialogComponent, {
        height: '300px',
        width: '270px',
        id: actionType + '-Stage',
        data: {
  ...
  private getTargetStage(stage: Stage, reverse: boolean): Stage {
    ...
    this.getStages(reverse).some((lookupStage: Stage) => {
      if (stage.id == lookupStage.id) return true;
      else result = lookupStage;
    });
    return result;
  }
  ...
  moveStage(stage: Stage, reverse = false) {
    stage.isLoading = true;
    if (this.disallowMove(stage, reverse)) return;
    ...
    this.stageServiceProxy
      .updateStageSortOrder(
        new UpdateSortOrderInput({
          id: stage.id,
          sortOrder: sortOrder,
        })
      )
  ...
  changeWidth(stage: Stage, direction: -1 | 1) {
    ...
    this.cacheService.set(cacheKey, stagesWidths);
  ...
  showChecklistEntityDialog(event, entity) {
    this.dialog.closeAll();
    this.dialog
      .open(EntityCheckListDialogComponent, {
        panelClass: ['slider'],
        hasBackdrop: false,
        closeOnNavigation: true,
        data: {
      ...
      .afterClosed()
      .subscribe(isUpdated => {
        if (isUpdated) this.refresh(entity.StageId);
      });
  ...
  getMaxColumnHeight(elm) {
    ...
    (this.stages || []).forEach(stage => {
      ...
      if (maxColumnHeight < height) maxColumnHeight = height;
    });
