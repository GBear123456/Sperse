...
export class LayoutService {
  ...
  public crmMenuPosition$ = new BehaviorSubject<'left' | 'right'>('left');
  ...
  displayDefaultPageHeader(value: boolean = false) {
    this.showPageLogo =
      this.showPlatformSelectMenu =
      this.showNotificationsButton =
      this.showChatButton =
  ...
  getMapPalette(): string[] {
    return (
      this.mapPalette[this.appSessionService.layoutType] || this.mapPalette[LayoutType.Default]
    );
  }
  ...
  getWelcomePageUri(): string {
    ...
    if (!pageUri) pageUri = AppConsts.defaultWelcomePageUri;
    return pageUri;
  }

  setCrmMenuSide(side: 'left' | 'right') {
    this.crmMenuPosition$.next(side);
  }

  getCrmMenuSide(): 'left' | 'right' {
    return this.crmMenuPosition$.getValue();
  }
}
