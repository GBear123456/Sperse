...
@Component({
  templateUrl: './leftbar-notifications.component.html',
  styleUrls: [
    '../../../../../shared/metronic/m-card-user.less',
    './leftbar-notifications.component.less',
  ],
  selector: 'leftbar-notifications',
  encapsulation: ViewEncapsulation.None,
  providers: [InstanceServiceProxy, TenantSubscriptionServiceProxy],
})
export class LeftbarNotificationsComponent implements OnInit {
  ...
  private readonly COMMUNICATION_MESSAGE_ENTITY_TYPE =
    'Sperse.CRM.Contacts.Communication.CommunicationMessage';
  ...
  getSubscriptionInfo() {
    ...
    if (this.appService.checkSubscriptionIsTrial()) {
      ...
      this.subscriptionInfoTitle =
        this.ls.l('YourTrialWillExpire', subscriptionName) +
        ' ' +
        (!dayCount
          ? this.ls.l('Today')
          : dayCount === 1
          ? this.ls.l('Tomorrow')
          : 'in ' + dayCount.toString() + ' ' + this.ls.l('Periods_Day_plural')
        ).toLowerCase() +
        '!';
      ...
      this.subscriptionInfoTitle = this.ls.l(
        'ModuleExpired',
        subscriptionName,
        this.appService.getSubscriptionStatusByModuleName()
      );
      this.subscriptionInfoText = this.ls.l(
        'GracePeriodNotification',
        (dayCount
          ? dayCount === 1
            ? this.ls.l('Tomorrow')
            : this.ls.l('PeriodDescription', dayCount.toString(), this.ls.l('Periods_Day_plural'))
          : this.ls.l('Today')
        ).toLowerCase()
      );
      ...
      this.subscriptionInfoTitle = this.ls.l('YouAreUsingPlan', subscriptionName);
    }
  ...
  loadNotifications(): void {
    this.notificationService
      .getUserNotifications(UserNotificationState.Unread, undefined, undefined, 3, 0)
  ...
  checkLocalizations(items: UserNotificationDto[]): Observable<boolean> {
    ...
    items
      .filter(
        v => v.notification.data.type == 'Abp.Notifications.LocalizableMessageNotificationData'
      )
      .forEach(v => {
    ...
    if (localizationSources.size == 0) return of(true);

    let calls: Observable<boolean>[] = [];
    localizationSources.forEach(v =>
      calls.push(this.localizationResolver.checkLoadLocalization(v))
    );
  ...
  registerToEvents() {
    ...
    abp.event.on('app.notifications.read', userNotificationId => {
      ...
      if (this.unreadNotificationCount <= 0) this.loadNotifications();
      else {
  ...
  subscriptionStatusBarVisible(): boolean {
    return (
      !this.appService.hasUnconventionalSubscription() &&
