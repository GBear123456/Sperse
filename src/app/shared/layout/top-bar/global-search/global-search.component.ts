...
export class GlobalSearchComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private getLeadGroup(
        ...
        return this.permissionService.checkCGPermission(
  ...
  valueChanged(e) {
    ...
    search(e) {
        ...
        if (
            !this.isCRMModule ||
            (this.showSearchMode &&
                this.selectedSearchMode != this.globalSearchPlaceholder)
        ) {
  ...
  getContactPhotoUrl(pictureId: string): string {
    ...
    tooltipReady(e) {
        e.component._$popupContent[0].classList.add(
    ...
    clearSearch(event?) {
        ...
        if (event) {
            if (
                ...
                if (
                    this.searchConfig &&
                    this.searchConfig.options.onValueChanged
                )
                    this.searchConfig.options.onValueChanged({ value: "" });
            } else {
