...
export class GlobalSearchComponent implements OnInit, OnDestroy {
  ...
  @HostBinding('class.showSearchMode') showSearchMode =
    this.toolbarService.isSearchBoxEnabled && this.layoutService.showLeftBar && this.isCRMModule;
  ...
  ngOnInit() {
    this.toolbarService.isSearchBoxEnabled$
      .pipe(takeUntil(this.lifecycleService.destroy$), debounceTime(600))
      .subscribe(isSearchBoxEnabled => {
        this.clearSearch();
        this.showSearchMode =
          isSearchBoxEnabled && this.layoutService.showLeftBar && this.isCRMModule;
        this.changeDetectorRef.detectChanges();
      });

    this.toolbarService.latestSearchConfig$
      ...
      .subscribe(config => {
        ...
        if (config) {
          if (config.options.value) {
            ...
            setTimeout(() => this.textBox.instance.option('value', config.options.value), 1000);
          }
        }
      });
  ...
  get itemsFound() {
    return !!(
      this.searchGroups &&
      this.searchGroups.some((searchGroup: GlobalSearchGroup) => {
  ...
  private getClientsGroup(search: string): Observable<GlobalSearchGroup> {
    return this.getGlobalSearchGroup(
      this.oDataService.getODataUrl('Contact', [{ ParentId: { eq: null } }]),
      this.ls.l('Customers'),
      'app/crm/clients',
      search,
      AppPermissions.CRMCustomers,
      [ClientFields.Id, ClientFields.Name, ClientFields.Email, ClientFields.PhotoPublicId],
      {
  ...
  private getPartnersGroup(search: string): Observable<GlobalSearchGroup> {
    return this.getGlobalSearchGroup(
      ...
      [PartnerFields.Id, PartnerFields.Name, PartnerFields.Email, PartnerFields.PhotoPublicId],
      {
  ...
  search(e) {
    ...
    if (
      !this.isCRMModule ||
      (this.showSearchMode && this.selectedSearchMode != this.globalSearchPlaceholder)
    ) {
      if (this.searchConfig && this.searchConfig.options.onValueChanged)
        this.searchConfig.options.onValueChanged({ value });
    } else if (value) this._search.next(value);
  }
  ...
  clearSearch(event?) {
    if (event && (event.name != 'value' || event.value)) return;
