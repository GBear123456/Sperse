...
import { Subscription, ReplaySubject, of, Observable, Subject } from 'rxjs';
...
export class ClientsNavigationComponent extends AppComponentBase implements OnInit {
  ...
  @ViewChild('searchInput') searchInput: DxTextBoxComponent;
  ...
  constructor(injector: Injector, private router: Router, private appService: AppService) {
    super(injector);
  ...
  loadContact() {
    ...
    this.contactDropdownDataSource.load().then((contacts: any[]) => {
      ...
      if (this.router.url.indexOf('lead') < 0 && segments.indexOf('company') < 0) {
        ...
        if (contact[this.clientFields.OrganizationId]) this.navigateToContact(routeId);
      }
  ...
  navigateToContact(contactId: number) {
    ...
    if (contact[this.clientFields.OrganizationId]) {
      if (companyIdIndex < 0) {
        ...
        segments[companyIdIndex + 1] = contact[this.clientFields.OrganizationId]?.toString();
      }
    } else {
      if (companyIdIndex >= 0) {
        segments = [...segments.slice(0, contactIndex + 2), ...segments.slice(companyIdIndex + 2)];
      }
    }
  ...
  toggleSearch() {
    ...
    setTimeout(() => {
      if (this.searchInput && this.isSearchMode) {
  ...
  onInputSearch(e) {
    this.searchValue = e.event.target.value;
  }

  search(e) {
    const value = e.component.option('value')?.toLowerCase() || '';
    ...
    this.contacts = this.contacts.filter(contact => {
      const name = (contact[this.clientFields.Name] || '').toLowerCase();
      const email = (contact[this.clientFields.Email] || '').toLowerCase();
      const company = (contact[this.clientFields.CompanyName] || '').toLowerCase();
      return name.includes(value) || email.includes(value) || company.includes(value);
    });
    this.contactIds = this.contacts.map(c => c[this.clientFields.Id]);
    this.isDropdownVisible = true;
  }
