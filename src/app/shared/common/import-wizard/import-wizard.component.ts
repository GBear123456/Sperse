...
export class ImportWizardComponent extends AppComponentBase implements AfterViewInit {
  ...
  constructor(
    ...
    this.dataMapping = formBuilder.group({
      valid: [
        ...
        () => {
          ...
          return validationResult && validationResult.isMapped && !validationResult.error
            ? null
            : validationResult;
        },
      ],
    ...
    this.store$
      .pipe(select(CountriesStoreSelectors.getCountries), filter(Boolean), first())
  ...
  next() {
    if (this.stepper.selectedIndex == this.UPLOAD_STEP_INDEX) {
      ...
      if (this.uploadFile.valid) this.buildMappingDataSource().then(() => this.stepper.next());
      else this.message.error(this.l('ChooseCorrectCSV'));
  ...
  getMappedFields() {
    let mappedFields = (this.mapGrid && this.mapGrid.instance.getSelectedRowsData()) || [];
    if (!mappedFields.length) {
      mappedFields =
        (this.mapDataSource &&
          this.mapDataSource.store &&
  ...
  complete(rows = null, importAll = false) {
    if (rows && !rows.length) return this.message.info(this.l('Import_NoRecordsAvailable'));

    let data = rows || this.reviewGrid.instance.getSelectedRowsData();
    this.onComplete.emit({
      fields: this.fileHeaderWasGenerated
        ? undefined
  ...
  showFinishStep() {
    this.stepper.selectedIndex = this.selectedStepIndex = this.FINISH_STEP_INDEX;
  }
  ...
  initReviewDataSource(mappedFields) {
    ...
    setTimeout(() => {
      let dataSource = [],
        progress = 0,
        totalCount = this.fileData.data.length - (this.fileHasHeader ? 0 : 1),
        mappedFieldsSorted = mappedFields
          .slice()
          .sort((prev, next) => prev.mappedField.localeCompare(next.mappedField)),
          ...
          if (index) {
            if (row.length == columnCount) {
              ...
              mappedFieldsSorted.forEach(field => {
                ...
                if (
                  !(
                    this.preProcessFieldBeforeReview &&
                    this.preProcessFieldBeforeReview(field, value, data)
                  ) &&
                  !data[field.mappedField]
                )
                  data[field.mappedField] = value;
              });
  ...
  updateGroupNames() {
    ...
    this.reviewGroups.forEach(group => {
      if (group && group.length) {
        ...
        } else item.compared = this.l('Unique records');
  ...
  checkDuplicate(raw, row) {
    ...
    return !(row.uniqueIdent = ident) || (this.duplicateCounts[ident] = (count || 0) + 1) > 1;
  }
    ...
    if ((newGroupIndex = this.reviewGroups.length)) {
      ...
      this.checkSimilarFields.forEach(fields => {
        fields.forEach(field => {
          let conditionValues = field.split(':').map(fld => {
            ...
            fieldIndex = conditionValues.every(Boolean) ? this.getUniqueIdent(conditionValues) : 0;
          if (fieldIndex) {
            ...
            if (groupIndex) {
              ...
              if (group && group.length == 1) group[0].highliteFields = conditionValues;
            } else this.similarFieldsIndex[fieldIndex] = newGroupIndex;
          }
        });
      });
      ...
      if (mergeGroupsIndex.length) {
        mergeGroupsIndex.forEach(index => {
          if (this.reviewGroups[index]) newGroup = newGroup.concat(this.reviewGroups[index]);
  ...
  highlightUnmappedFields(rows) {
    rows.forEach(row => {
      if (!row.mappedField) this.highlightUnmappedField(row);
    });
  }
  ...
  highlightUnmappedField(row) {
    ...
    let rows = cellElement
      ? cellElement
          .closest('.dx-datagrid-rowsview')
          .querySelectorAll(`tr:nth-of-type(${rowIndex + 1})`)
      : [];
  ...
  parse(content) {
    ...
    this.parser.parse(this.fileContent, {
      complete: (results: PapaParseResult) => {
        ...
        if (!this.checkFileDataValid()) this.message.error(this.l('IncorrectFileFormatError'));
        else {
  ...
  loadFileContent(file) {
    ...
    reader.onprogress = event => {
      if (event.total > event.loaded)
        this.dropZoneProgress = Math.round((event.loaded / event.total) * 100);
    };
  ...
  fileDropped(files: NgxFileDropEntry[]) {
    ...
    if (this.files.length) {
      ...
      if (file.fileEntry) file.fileEntry['file'](this.loadFileContent.bind(this));
      else this.loadFileContent(file);
    }
  }
        ...
        let createDataSourceInternal = (mappingSuggestions = []) => {
          let noNameCount = 0,
            ...
            data: any[] = this.fileData.data[0].map((field, index) => {
              ...
              if (field) {
                if (isNaN(usedSourceFields[field])) usedSourceFields[field] = 1;
                else {
                ...
                let suggestionField =
                  _.findWhere(mappingSuggestions, { inputFieldName: field }) ||
                  mappingSuggestions.find(
                    v => this.normalizeFieldName(v.inputFieldName) == normalizedFieldName
                  );
                if (suggestionField) {
                  foundItem = this.lookupFields.find(
                    item => suggestionField.outputFieldName == item.id
                  );
                }
                ...
                if (foundItem) usedMapFields[foundItem.id] = true;
              }
          ...
          this.mapField(
            ...
            s =>
              ...
              s.replace('contactcustom', 'custom').replace('contact', 'custom'),
            usedMapFields
          );
          ...
          this.mapField(
            data,
            s => 'businessinfo' + s.replace('companyaddress', 'companyfulladdress'),
            usedMapFields
          );
          this.mapField(
            data,
            s => 'businessinfo' + s.replace('company', 'companyfulladdress'),
            usedMapFields
          );
          this.mapField(
            data,
            s => 'businessinfo' + s.replace('company', 'companyfulladdress') + 'name',
            usedMapFields
          );
          this.mapField(
            data,
          ...
          this.mapField(
            data,
            s => 'businessinfo' + s.replace('workaddress', 'workfulladdress'),
            usedMapFields
          );
          this.mapField(
            data,
            s => 'businessinfo' + s.replace('work', 'workfulladdress'),
            usedMapFields
          );
          this.mapField(
            data,
            s => 'businessinfo' + s.replace('work', 'workfulladdress') + 'name',
            usedMapFields
          );
          this.mapField(
            data,
          ...
          this.mapField(
            ...
            s =>
              'requestcustominfo' +
              s
                .replace('requestcustom', 'custom')
                .replace('request', 'custom')
                .replace('leadcustom', 'custom')
                .replace('lead', 'custom'),
            usedMapFields
          );
          ...
          data.forEach(v => {
            if (v.mappedField) selectedMapRowKeys.push(v.id);
          });
          this.selectedMapRowKeys = selectedMapRowKeys;
          resolve();
        };

        if (this.fileHeaderWasGenerated) createDataSourceInternal();
        else
          this.getColumnsMappingSuggestions(res => {
  ...
  normalizeFieldName(name: string): string {
    if (name) return name.replace(/\s|_|-|\./g, '').toLowerCase();

    return null;
  }

  findFieldInLookupFields(
    field: string,
    usedMapFields,
    lookupFieldModifyFunc: (s: string) => string = null
  ) {
    if (lookupFieldModifyFunc) {
      return this.lookupFields.find(
        item => !usedMapFields[item.id] && lookupFieldModifyFunc(item.normalizedId) == field
      );
    } else {
  ...
  mapField(
    data: any[],
    sourceFieldModifyFunc: (s: string) => string,
    usedMapFields,
    lookupFieldModifyFunc: (s: string) => string = null
  ) {
    data.forEach(fieldData => {
      if (!fieldData.mappedField && fieldData.normalizedSourceField) {
        ...
        let foundItem = this.findFieldInLookupFields(
          fieldToFind,
          usedMapFields,
          lookupFieldModifyFunc
        );
    ...
    if (this.fileData.data.length) {
      ...
      for (let i = 0; i < constNames.length; i++) {
        ...
        this.fileData.data[0].forEach((val: string) => {
          if (val.toLowerCase().indexOf(constNames[i]) != -1) nameIsPresent = true;
        });
        if (nameIsPresent) {
          namesFoundCount++;
        }
  ...
  downloadFromURL() {
    if (!this.uploadFile.get('url').invalid) {
      ...
      else this.message.error(this.l('FieldEmptyError', [this.l('URL')]));
    }
  }
  ...
  onRowValidating($event) {
    if ($event.newData.mappedField) {
      ...
      this.mapDataSource.store.data.forEach(row => {
        if (
          $event.oldData.sourceField != row.sourceField &&
          $event.newData.mappedField &&
    ...
    } else $event.component.deselectRows([$event.key]);
  }

  selectionModeChanged($event) {
    this.reviewGrid.instance.option('selection.selectAllMode', $event.itemData.mode);
  }
  ...
  onMapSelectionChanged($event) {
    setTimeout(() => {
      ...
      $event.selectedRowsData.forEach(row => {
        if (!row.mappedField) rowIdsToDeselect.push(row.id);
      });

      if (rowIdsToDeselect.length) $event.component.deselectRows(rowIdsToDeselect);
  ...
  onLookupFieldsItemRendered($event) {
    if (this.mapGrid)
      this.mapGrid.instance.getSelectedRowsData().forEach(row => {
        if (row.mappedField == $event.node.key) $event.itemElement.classList.add('mapped');
      });
  }
  ...
  onItemClick(event, cell) {
    if (event.node.key != cell.value) cell.setValue(event.node.key);
  }
  ...
  customizePreviewColumns = columns => {
    ...
    columns.forEach((column, index) => {
      ...
      if (column.dataField == 'compared') {
        if (this.reviewGroups.length) column.groupIndex = 0;
        else {
          column.visible = false;
          this.selectedPreviewRows = [];
        }
      } else {
        if (columnConfig) _.extend(column, columnConfig);
        this.initColumnTemplate(column);
        column.calculateDisplayValue = this.calculateDisplayValue;
        column.visibleIndex = _.findIndex(
          mappedFileds,
          item => column.dataField == item.mappedField
        );
  ...
  validateRowFields(data) {
    this.validateFieldList.forEach(fld => {
      Object.keys(data).forEach(field => {
        if (
          field.toLowerCase().includes(fld.toLowerCase()) &&
  ...
  addInvalidField(key, field) {
    ...
    if (invalidRowFields && invalidRowFields.indexOf(field) < 0) invalidRowFields.push(field);
    else this.invalidRowKeys[key] = [field];
  }

  checkFieldValid(key, data, field): boolean {
    let value = data[field] && data[field].trim ? data[field].trim() : data[field];
    if (!value) return true;
    if (key == 'date' || key == 'dob') return !isNaN(Date.parse(value));
    if (key == 'phone') {
      let isValid = this.phoneNumberService.isPhoneNumberValid(
        value,
        this.getFieldCountryCode(data, field)
      );
      if (isValid)
        data[field] = this.phoneFormat
          .transform(value, this.getFieldDefaultCountry(data, field))
          .replace(/[^\d+]/g, '');
      return isValid;
    } else if (key == 'revenue')
      return !value || !isNaN(value) || !isNaN(parseFloat(value.replace(/[^0-9.]/g, '')));
    else if (key == 'countryName') return value.trim().length > 3;
    else if (key == 'countryId') {
      value = data[field] = value.toUpperCase();
      return (
        this.excludeCCValidation.indexOf(value) >= 0 ||
        !!_.findWhere(this.countries, { code: value })
      );
    } else if (key == 'stateId') {
      value = data[field] = value.toUpperCase();
      return value.length >= 2 && value.length <= 3;
    } else if (key == 'rating') return !isNaN(value) && value >= 1 && value <= 10;
    else if (key == 'gender')
    ...
    else return AppConsts.regexPatterns[key].test(value);
  }
  ...
  getFieldDefaultCountry(data, field) {
    ...
    if (countryCode) return { defaultCountry: countryCode };
  }
  ...
  getFieldCountryCode(data, field) {
    ...
    if (country) {
      ...
      if (contryFieldValue) return this.getCountryCodeByCountryName(contryFieldValue);
    }
  }
  ...
  calculateDisplayValue(data) {
    ...
    if (value && value.length > MAX_LEN) return value.substr(0, MAX_LEN) + '...';
    return value;
  }
  ...
  selectedStepChanged(event) {
    ...
    if (event) this.selectedStepIndex = event.selectedIndex;
  }
}
