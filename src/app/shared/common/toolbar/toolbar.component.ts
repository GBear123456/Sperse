...
export class ToolBarComponent implements OnDestroy, OnInit {
    ...
    private subscription: Subscription =
        this.filtersService.filterToggle$.subscribe((enabled: boolean) => {
            enabled ||
    ...
    private toolbarItemAction(
        item: ToolbarGroupModelItem,
        group: ToolbarGroupModel,
        event: any
    ) {
        if (item.action) item.action.call(this, event);
        if (group.areItemsDependent)
            group.items.forEach((i) => {
                $(".dx-button[accesskey=" + i.name + "]").removeAttr(
                    "button-pressed"
                );
            });

        if (
            (item.options && item.options["checkPressed"]) ||
            item["checkVisible"]
        ) {
            this.initToolbarItems();
            this.changeDetectorRef.detectChanges();
    ...
    getOptions() {
        let option = document.querySelector(
            ".toolbar-download-options input:checked"
        );
    ...
    getElementAttr(item) {
        if (item.name == "select-box") {
            ...
            return {
                ...
                "select-value":
                    items && items.length && selectedItem
                        ? selectedItem.text
                        : "",
            };
        }
        ...
        return (
            ...
            (!item.options ||
                ...
                item.options.items.some(
                    (subitem) =>
                        !subitem.hasOwnProperty("visible") || subitem.visible
                )) &&
    ...
    initToolbarItems() {
        let supportedButtons = this.getSupportedButtons(),
            ...
            items = [];
        if (this._config)
            this._config.forEach(
                (group: ToolbarGroupModel, configIndex: number) => {
                    let groupItems = group.items.filter((item) =>
                            this.checkItemVisible(item)
                        ),
                        count = groupItems.length;
                    groupItems.forEach(
                        (item: ToolbarGroupModelItem, index: number) => {
                            ...
                            this.checkItemVisible(item) &&
                                items.push({
                                    ...
                                    itemTemplate:
                                        item.itemTemplate || group.itemTemplate,
                                    template:
                                        item.itemTemplate ||
                                        mergedConfig.itemTemplate,
                                    options: _.extend(
                                        {
                                            focusStateEnabled: true,
                                            onClick: (e) =>
                                                this.toolbarItemAction(
                                                    item,
                                                    group,
                                                    e
                                                ),
                                            elementAttr: _.extend(
                                                {
                                                    ...
                                                    "group-item-position":
                                                        ToolbarService.getGroupItemPosition(
                                                            index,
                                                            count
                                                        ),
                                                    "group-item-count": count,
                                                    "group-item-index":
                                                        ToolbarService.getGroupItemIndex(
                                                            index,
                                                            count
                                                        ),
                                                },
    ...
    updateToolbarItemAttribute(itemName: string, property: string, value: any) {
        const toolbarItemIndex = this.items.findIndex(
