...
export class SettingsNewComponent extends AppComponentBase implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.router.events.pipe(filter(event => event instanceof NavigationEnd)).subscribe(event => {
      this.selectedMenu = this.settingService.configuredNavs.find(nav =>
        location.pathname.startsWith(this.settingService.getFullPath(nav.path) || '')
      );
      this.selectedMainItem = this.selectedMenu?.id;
      this.settingService.alterSubmenu(
        this.selectedMenu?.submenu && this.selectedMenu.submenu.length > 0
      );

      if (location.pathname !== this.settingService.getFullPath('dashboard')) {
        if (this.selectedMenu.submenu) {
          const subMenu = this.selectedMenu.submenu.find(sub =>
            location.pathname.startsWith(this.settingService.getFullPath(sub.path) || '')
          );
          if (subMenu?.submenu) {
            this.title = subMenu.submenu.find(
              sub => this.settingService.getFullPath(sub.path) === location.pathname
            )?.label;
