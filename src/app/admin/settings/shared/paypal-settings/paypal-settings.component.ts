...
export class PaypalSettingsComponent extends SettingsComponentBase {
  ...
  faqs = [
    {
      ...
      content:
        'Sign in to your PayPal Developer Dashboard at developer.paypal.com. Create a new app under "My Apps & Credentials" to get your API credentials. Then enter these credentials in the API Settings tab.',
    },
    {
      title: "What's PayPal and how do I get started?",
      content:
        'PayPal is a global payment processor that lets customers pay using credit cards or their PayPal account balance. Sign up at paypal.com to create a business account, then visit developer.paypal.com to access API credentials.',
    },
  ];
  ...
  loadSettings() {
    ...
    if (this.isPaymentsEnabled) {
      this.tenantPaymentSettingsService
        .getPayPalSettings()
  ...
  createConnectedAccount() {
    if (
      this.isHost ||
      !this.paypalPaymentSettings.isHostAccountEnabled ||
      (this.paypalPaymentSettings && this.paypalPaymentSettings.merchantId)
    )
      return;
  ...
  unlinkMerchant() {
    if (this.isHost || !this.paypalPaymentSettings.merchantId) return;

    this.message.confirm('', this.l('Do you want to unlink Paypal account ?'), isConfirmed => {
      if (isConfirmed) {
        this.startLoading();
        this.tenantPaymentSettingsService
          .unlinkPayPalMerchant()
          .pipe(finalize(() => this.finishLoading()))
          .subscribe(() => {
  ...
  get isMerchantConnected(): boolean {
    return (
      this.paypalPaymentSettings.merchantId &&
      this.paypalPaymentSettings.merchantEmailConfirmed &&
      this.paypalPaymentSettings.merchantPaymentsReceivable
    );
  }
