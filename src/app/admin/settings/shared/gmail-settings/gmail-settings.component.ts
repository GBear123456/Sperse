...
@Component({
  ...
  styleUrls: ['./gmail-settings.component.less', '../settings-base.less'],
...
export class GmailSettingsComponent extends SettingsComponentBase {
    ...
    this.googleService
      ...
      .subscribe(res => {
        ...
        this.gmailSettingsService.initGmailClient(this.gmailSettings.clientId, response => {
          this.startLoading();
          this.googleService
            .setupGmail(false, response.code)
            .pipe(finalize(() => this.finishLoading()))
            .subscribe(() => {
              this.initClient();
            });
  ...
  sendTestEmail(): void {
    if (!this.gmailSettings.isConfigured) return;

    this.gmailSettingsService.sendTestEmail(
      this.testEmailAddress,
      this.gmailSettings.defaultFromAddress,
      this.gmailSettings.defaultFromDisplayName,
      false
    );
  }
  ...
  getSaveObs(): Observable<any> {
    ...
    obj.forUser = false;
    return this.googleService.updateGmailSettings(obj).pipe(
      tap(() => {
