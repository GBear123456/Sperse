...
export class CountryService {
  ...
  getPhoneCodeByCountryCode(countryCode: string): string {
    let countries = this.getCountries(), phoneCode;
    countries.some(country => {
      if (country.countryCode.localeCompare(countryCode, 'en', { sensitivity: 'base' }) == 0)
        return phoneCode = '+' + country.dialCode;
    });
    return phoneCode;
  }
  ...
  private loadCountries(locale: string = 'en'): Country[] {
    ...
    countries.forEach((country: Country) => {
      country.name = LOCALES[locale][country.countryCode].toLowerCase().replace(/\b(\w)/g, s => s.toUpperCase());
    });
    return countries;
  }
}
