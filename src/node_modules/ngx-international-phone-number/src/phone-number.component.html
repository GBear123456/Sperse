<div class="input-group">
    ...
    (mousedown)="keyDown($event)" (keydown)="keyDown($event)" (keyup)="keyUp($event)" (focus)="focusIn($event)" (blur)="focusOut($event)" (paste)="paste($event)"
    [placeholder]="placeholder" *ngIf="required" required>

    <! -- if not required -->
    <input onlyNumber="true" class="form-control" [attr.maxlength]="maxlength" [(ngModel)]="phoneNumber" (ngModelChange)="updatePhoneNumber($event)"
    (mousedown)="keyDown($event)" (keydown)="keyDown($event)"  (keyup)="keyUp($event)" (focus)="focusIn($event)" (blur)="focusOut($event)" (paste)="paste($event)"
    [placeholder]="placeholder" *ngIf="!required">
</div>
