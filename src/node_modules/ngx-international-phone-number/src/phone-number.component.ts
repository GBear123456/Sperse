...
export class PhoneNumberComponent implements OnInit, ControlValueAccessor, Validator {
    ...
    showDropdown = false;
    phoneNumber = '';
    ignoreCountryOnChange: boolean = false;
    ...
    private reducePrefixes(foundPrefixes: Country[]) {
        return foundPrefixes.reduce((first: Country, second: Country) => {
            if (first.dialCode.length > second.dialCode.length)
                return first;
            else if (second.dialCode.length > first.dialCode.length
                || second.countryCode == this.defaultCountry
            ) return second;
            else
                return first;
        });
    ...
    displayDropDown() {
        if (this.allowDropdown && !this.disabled) {
            ...
            if (!this.showDropdown)
                this.onDropDownHover.emit(false);
        }
    }
    ...
    private findPrefix(prefix: string) {
        let cleanPrefix = prefix.replace(/[\s\(\)\-]/g, ''),
            foundPrefixes: Country[] = this.countries.filter(
            (country: Country) => cleanPrefix.startsWith(country.dialCode)
        );
    ...
    writeValue(value: string) {
        ...
        this.ignoreCountryOnChange = false;
    ...
    validate(c: FormControl): ValidationErrors | null {
        ...
        if (value) {
            ...
            try {
                let phoneNumber = phoneUtil.parse(value, this.defaultCountry.toUpperCase());
    ...
    dropDownHover(state: boolean) {
        this.onDropDownHover.emit(state);
    }
     ...
     */
    private updateValue(ignoreCountryOnChange: boolean = false) {
        this.value = this.getCleanNumber(this.phoneNumber, false);
        if (!PhoneNumberComponent.startsWithPlus(this.value))
            this.value = AppConsts.defaultCountryPhoneCode + this.value;

        this.ignoreCountryOnChange = ignoreCountryOnChange;
