...
export class PhoneNumberComponent implements OnInit, ControlValueAccessor, Validator {
        ...
        if (event.key) {
            if(/\D/.test(event.key)) {  
                ...
                if (event.keyCode == 8/* Backspace */)
                    input.value = this.getInputMaskValue(
                        this.getPhoneValue(input.value).slice(0, -1))
    ...
    private updateCaretPosition(input) {
        ...
        if (caretPosition > 0)
            setTimeout(() => this.setCaretPosition(input, caretPosition));
    }

    private getCleanNumber(value) {
     ...
     */
    private updateValue() {
        this.value = this.getCleanNumber(this.phoneNumber);
        this.onModelChange(this.value);
        this.onTouch();
    }

    private getPhoneValue(phoneNumber) {
        phoneNumber = this.getCleanNumber(phoneNumber);
        return PhoneNumberComponent.startsWithPlus(phoneNumber)
            ? `${phoneNumber.split(PLUS)[1].substr(this.selectedCountry.dialCode.length, phoneNumber.length)}`
            : phoneNumber;
    }
    ...
    private updatePhoneInput(countryCode: string) {
        ...
        let newInputValue: string = this.getPhoneValue(this.phoneNumber);
        this.selectedCountry = this.countries.find((country: Country) => country.countryCode === countryCode);
        if (this.selectedCountry)
            this.phoneNumber = this.getInputMaskValue(newInputValue);//`${PLUS}${this.selectedCountry.dialCode} ${newInputValue.replace(/ /g, '')}`;
        else
