...
export class PhoneNumberComponent implements OnInit, ControlValueAccessor, Validator {
    ...
    private findPrefix(prefix: string) {
        ...
        ), defaultCountry = foundPrefixes.filter(
            (country: Country) => country.countryCode == this.defaultCountry
        );

        if (foundPrefixes && foundPrefixes.length)
            this.selectedCountry = defaultCountry.length ? defaultCountry.pop()
                : PhoneNumberComponent.reducePrefixes(foundPrefixes);
        else
            this.selectedCountry = null;
    }
