...
(function (factory) {    
    ...
    jQCalendar.init = function (obj, embeded, options) {
        ...
        calendarObj.dateRangePicker(
            Object.assign({
                ...
                customTopBar: 
                    ...
                    '<input class="topBarStartDate" placeholder="dd/mm/yyyy"></input>' +
                    '<i class="fa fa-arrow-right arrowToDate" aria-hidden="true"></i>' +
                    '<i class="fa fa-calendar-o" aria-hidden="true"></i>' +
                    '<input class="topBarEndDate" placeholder="dd/mm/yyyy"></input>' +
                    '</div>',
                ...
                // }
                $(".topBarStartDate").val(moment(obj.date1).format("DD/MM/YYYY"));
            })
            .bind('datepicker-open', function (event) {
                var startInput = $(".topBarStartDate"), endInput = $(".topBarEndDate"),
                    ...
                    format = "DD/MM/YYYY";
                inputmask("date", { jitMasking: true }).mask(startInput);
                inputmask("date", { jitMasking: true }).mask(endInput);
            ...
            .bind('datepicker-change', function (event, obj) {
                ...
                $(".topBarStartDate").val(moment(obj.date1).format("DD/MM/YYYY"));
                $(".topBarEndDate").val(moment(obj.date2).format("DD/MM/YYYY"));
        ...
        $('.date-picker-wrapper .allAvailableDates').on('click', function (e) {
            ...
            var endDate = options.allowFutureDates ? moment(calendarObj.data('dateRangePicker').getLastDate()).toDate()
                : (calendarObj.data('dateRangePicker').getAllowCurrentDate() ? moment().toDate() : moment().subtract({ Days: 1 }).toDate());
            startDate.setTime(startDate);
            endDate.setTime(endDate);
            calendarObj.data('dateRangePicker').setDateRange(startDate, endDate);
        });
