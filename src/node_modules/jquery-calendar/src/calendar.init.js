...
(function (factory) {
    ...
    jQCalendar.init = function (obj, embeded, options) {
        ...
        $('.date-picker-wrapper .allAvailableDates').on('click', function (e) {
            e.stopPropagation();
            var dateRangePicker = calendarObj.data('dateRangePicker');
            var startDate = getValidStartDate(dateRangePicker.getFirstDate());
            var endDate = dateRangePicker.getLastDate();
            startDate.setTime(startDate);
            endDate.setTime(endDate);
            dateRangePicker.setRangePreselectedFlag();
            dateRangePicker.setRangeSelectedPeriod('AllAvailableDates');
            dateRangePicker.setDateRange(startDate, endDate);
        });
        ...
        function getValidEndDate(expectedEndDate) {
            ...
            if (moment(expectedEndDate).isSameOrAfter(getUserTimezoneDate(new Date()))) {
                if (datePicker.getAllowCurrentDate() === false)
                    return getUserTimezoneDate(moment().subtract({ days: 1 }).toDate());
                else if (datePicker.getAllowFutureDates() === false)
                    return getUserTimezoneDate(moment().toDate());
            }
