export class StringHelper {
    ...
    static getBase64(data: string): string {
        let prefixIndex = 0, prefix = StringHelper.prefixes.find(p => (prefixIndex = data.indexOf(p)) !== -1);
        return data && data.slice(prefixIndex + prefix.length);
    }
    static getSize(originalSize: number, data: string): number {
        return data.indexOf(StringHelper.prefixes[0]) ? originalSize - 3 : originalSize;
    }
}
