...
export class DashboardWidgetsService {
    ...
    map((dateRange: CalendarValuesModel) => {
      return {
        from:
          ...
          DateHelper.removeTimezoneOffset(new Date(dateRange.from.value.getTime()), true, 'from'),
        to:
          dateRange.to.value &&
          DateHelper.removeTimezoneOffset(new Date(dateRange.to.value.getTime()), true, 'to'),
  ...
  totalsDataAvailable$: Observable<boolean> = this.totalsData$.pipe(
    map(
      (totalsData: GetTotalsOutput) =>
        !!(totalsData.totalOrderAmount || totalsData.totalLeadCount || totalsData.totalClientCount)
    )
  );
  ...
  totalsDataFields = [
    {
      ...
      type: 'currency',
      percent: '0%',
      visible: this.isAnyCGGranted || this.permissionService.isGranted(AppPermissions.CRMOrders),
      showDetails: true,
  ...
  private _contactGroupId: BehaviorSubject<ContactGroup> = new BehaviorSubject<ContactGroup>(
    this.permissionService.getFirstAvailableCG()
  );
  ...
  private _currencyIdSubject: BehaviorSubject<string> = new BehaviorSubject<string>(
  ...
  constructor(
    ...
    combineLatest(
      ...
      .pipe(
        ...
        switchMap(
          ([period, contactId, groupId, orgUnitIds, currencyId]: [
            PeriodModel,
            number,
            ContactGroup,
            number[],
